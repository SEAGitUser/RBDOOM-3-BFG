...
void idBrittleFracture::Restore( idRestoreGame* savefile )
{
	...
	for( int i = 0; i < numEvents; i++ )
	{
		...
		{
			ProjectDecal( restoredEvent.point, restoredEvent.vector, gameLocal.GetGameTimeMs(), NULL );
		}
		else
		{
			Shatter( restoredEvent.point, restoredEvent.vector, gameLocal.GetGameTimeMs() );
		}
		resolveBreaks = true;
	}
...
void idBrittleFracture::Spawn()
{
	...
	if( renderEntity.hModel != NULL )
	{
		...
		for( int i = 0; i < model->NumSurfaces(); ++i )
		{
			...
			if( idStr( surf->GetMaterial()->GetName() ) == "textures/smf/window_scratch" )
			{
				isXraySurface = true;
				break;
			}
...
void idBrittleFracture::RemoveShard( int index )
{
	...
	for( int i = index; i < shards.Num(); ++i )
	{
...
================
*/
bool idBrittleFracture::UpdateRenderEntity( renderEntityParms_t* renderEntity, const renderViewParms_t* renderView ) const
{
	...
	if( lastRenderEntityUpdate == gameLocal.GetGameTimeMs() || !changed )
	{
		return false;
	}
	
	lastRenderEntityUpdate = gameLocal.GetGameTimeMs();
	changed = false;
	...
	auto tris = renderEntity->hModel->AllocSurfaceTriangles( numTris * 3, material->ShouldCreateBackSides() ? numTris * 6 : numTris * 3 );
	auto decalTris = renderEntity->hModel->AllocSurfaceTriangles( numDecalTris * 3, decalMaterial->ShouldCreateBackSides() ? numDecalTris * 6 : numDecalTris * 3 );
	...
	for( i = 0; i < shards.Num(); ++i )
	{
		...
		{
			msec = gameLocal.GetGameTimeMs() - shards[i]->droppedTime - SHARD_FADE_START;
			if( msec > 0 )
			{
		...
		{		
			...
			v->Clear();
			v->SetPosition( origin + winding[ 0 ].ToVec3() * axis );
			...
			v = &tris->verts[ tris->numVerts++ ];
			v->Clear();
			v->SetPosition( origin + winding[ j - 1 ].ToVec3() * axis );
			...
			v = &tris->verts[ tris->numVerts++ ];
			v->Clear();
			v->SetPosition( origin + winding[ j ].ToVec3() * axis );
		...
		for( k = 0; k < shards[i]->decals.Num(); ++k )
		{
			...
			{			
				...
				v->Clear();
				v->SetPosition( origin + decalWinding[ 0 ].ToVec3() * axis );
				...
				v = &decalTris->verts[ decalTris->numVerts++ ];
				v->Clear();
				v->SetPosition( origin + decalWinding[ j - 1 ].ToVec3() * axis );
				...
				v = &decalTris->verts[ decalTris->numVerts++ ];
				v->Clear();
				v->SetPosition( origin + decalWinding[ j ].ToVec3() * axis );
...
================
*/
bool idBrittleFracture::ModelCallback( renderEntityParms_t* renderEntity, const renderViewParms_t* renderView )
{
	auto ent = static_cast<const idBrittleFracture*>( gameLocal.entities[ renderEntity->entityNum ] );
	if( ent == NULL )
	{
...
void idBrittleFracture::Think()
{
	...
	for( int i = 0; i < shards.Num(); ++i )
	{
		int droppedTime = shards[i]->droppedTime;
		if( droppedTime != -1 )
		{
			if( gameLocal.GetGameTimeMs() - droppedTime > SHARD_ALIVE_TIME )
			{
				RemoveShard( i );
				--i;
			}
	...
	{
		int startTime = gameLocal.GetPreviousGameTimeMs();
...
void idBrittleFracture::ApplyImpulse( idEntity* ent, int id, const idVec3& point, const idVec3& impulse )
{
	...
	{
		Shatter( point, impulse, gameLocal.GetGameTimeMs() );
	}
}
...
void idBrittleFracture::AddForce( idEntity* ent, int id, const idVec3& point, const idVec3& force )
{
	...
	{
		Shatter( point, force, gameLocal.GetGameTimeMs() );
	}
}
...
void idBrittleFracture::ProjectDecal( const idVec3& point, const idVec3& dir, const int time, const char* damageDefName )
{
	...
	if( time >= gameLocal.GetGameTimeMs() )
	{
...
void idBrittleFracture::DropShard( shard_t* shard, const idVec3& point, const idVec3& dir, const float impulse, const int time )
{
	...
	for( int i = 0; i < shard->neighbours.Num(); ++i )
	{
		auto neighbour = shard->neighbours[i];
		for( int j = 0; j < neighbour->neighbours.Num(); ++j )
		{
	...
	int clipModelId = shard->clipModel->GetId();
	physicsObj.SetClipModel( NULL, 1.0f, clipModelId, false );
	
	const idVec3 & origin = shard->clipModel->GetOrigin();
	const idMat3 & axis = shard->clipModel->GetAxis();
	
	// set the dropped time for fading
	shard->droppedTime = time;
	
	idVec3 dir2 = origin - point;
	float dist = dir2.Normalize();
	float f = dist > maxShatterRadius ? 1.0f : idMath::Sqrt( idMath::Fabs( dist - minShatterRadius ) ) * ( 1.0f / idMath::Sqrt( idMath::Fabs( maxShatterRadius - minShatterRadius ) ) );
...
void idBrittleFracture::Shatter( const idVec3& point, const idVec3& impulse, const int time )
{
	...
	if( time > ( gameLocal.GetGameTimeMs() - SHARD_ALIVE_TIME ) )
	{
...
void idBrittleFracture::DropFloatingIslands( const idVec3& point, const idVec3& impulse, const int time )
{
	...
	int numIslands = 0;
	auto queue = ( shard_t** ) _alloca16( shards.Num() * sizeof( shard_t** ) );
	for( int i = 0; i < shards.Num(); i++ )
	{
	...
	{	
		...
		for( auto curShard = queue[queueStart]; queueStart < queueEnd; curShard = queue[++queueStart] )
		{	
			for( int j = 0; j < curShard->neighbours.Num(); ++j )
			{	
				auto nextShard = curShard->neighbours[j];
...
void idBrittleFracture::AddDamageEffect( const trace_t& collision, const idVec3& velocity, const char* damageDefName )
{
	if( !disableFracture )
	{
		ProjectDecal( collision.c.point, collision.c.normal, gameLocal.GetGameTimeMs(), damageDefName );
	}
}
...
void idBrittleFracture::CreateFractures( const idRenderModel* renderModel )
{
	...
	auto surf = renderModel->Surface( 0 );
	...
	if( isXraySurface )
	{
		...
		for( int i = 0; i < 4; i++ )
		{
			const idDrawVert* v = &surf->GetTriangles()->verts[i];
			w.AddPoint( idVec5( v->GetPosition(), v->GetTexCoord() ) );
		}
	...
	{
		const idDrawVert* verts = surf->GetTriangles()->verts;
		triIndex_t* indexes = surf->GetTriangles()->indexes;
		
		for( int j = 0; j < surf->GetTriangles()->numIndexes; j += 3 )
		{
			int i0 = indexes[ j + 0 ];
			int i1 = indexes[ j + 1 ];
			int i2 = indexes[ j + 2 ];
			...
			idFixedWinding w;
			w.AddPoint( idVec5( verts[i2].GetPosition(), verts[i2].GetTexCoord() ) );
			w.AddPoint( idVec5( verts[i1].GetPosition(), verts[i1].GetTexCoord() ) );
			...
			for( int k = j + 3; k < surf->GetTriangles()->numIndexes && ( w.GetNumPoints() + 1 < MAX_POINTS_ON_WINDING ); k += 3 )
			{
				...
				idFixedWinding w2;
				w2.AddPoint( idVec5( verts[i5].GetPosition(), verts[i5].GetTexCoord() ) );
				w2.AddPoint( idVec5( verts[i4].GetPosition(), verts[i4].GetTexCoord() ) );
				w2.AddPoint( idVec5( verts[i3].GetPosition(), verts[i3].GetTexCoord() ) );
				idPlane p2;
				w2.GetPlane( p2 );
				if( p1 != p2 )
				{
