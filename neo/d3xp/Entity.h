...
public:
	...
	static const uint32		INVALID_PREDICTION_KEY = MAX_UNSIGNED_TYPE( uint32 );
...
public:
	...
	bool					GetJointWorldTransform( jointHandle_t, int currentTime, idVec3& offset, idMat3& axis );
	bool					GetJointTransformForAnim( jointHandle_t, int animNum, int currentTime, idVec3& offset, idMat3& axis ) const;

	virtual int				GetDefaultSurfaceType() const;
	virtual void			AddDamageEffect( const trace_t& collision, const idVec3& velocity, const char* damageDefName );
	void					AddLocalDamageEffect( jointHandle_t, const idVec3& localPoint, const idVec3& localNormal, const idVec3& localDir, const idDeclEntityDef*, const idMaterial* collisionMaterial );
...
private:
	...
	void 					Event_ClearJoint( jointHandle_t );
	void 					Event_SetJointPos( jointHandle_t, jointModTransform_t transform_type, const idVec3& pos );
	void 					Event_SetJointAngle( jointHandle_t, jointModTransform_t transform_type, const idAngles& angles );
	void 					Event_GetJointPos( jointHandle_t );
	void 					Event_GetJointAngle( jointHandle_t );
};
