...
void idGameLocal::Init()
{
	...
	// set up the aas
	auto dict = FindEntityDefDict( "aas_types" );
	if( dict == NULL )
	{
	...
	auto kv = dict->MatchPrefix( "type" );
	while( kv != NULL )
	{
		aasList.Append( idAAS::Alloc() );
...
void idGameLocal::RunTimeGroup2( idUserCmdMgr& userCmdMgr )
{
	...
	for( auto ent = activeEntities.Next(); ent != NULL; ent = ent->activeNode.Next() )
	{
