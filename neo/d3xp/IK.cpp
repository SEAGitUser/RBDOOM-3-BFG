...
bool idIK::Init( idEntity* self, const char* anim, const idVec3& modelOffset )
{
	...
	auto model = animator->ModelHandle();
	if( model == NULL )
	{
		gameLocal.Warning( "idIK::Init: IK for entity '%s' at (%s) has no model set.",
						   self->name.c_str(), self->GetPhysics()->GetOrigin().ToString( 0 ) );
...
bool idIK::SolveTwoBones( const idVec3& startPos, const idVec3& endPos, const idVec3& dir, float len0, float len1, idVec3& jointPos )
{
	...
	x = ( idMath::Square( length ) + idMath::Square( len0 ) - idMath::Square( len1 ) ) * ( 0.5f * lengthInv );
	y = idMath::Sqrt( idMath::Square( len0 ) - idMath::Square( x ) );
...
void idIK_Walk::Evaluate()
{
	...
	animator->CreateFrame( gameLocal.GetGameTimeMs(), false );
	...
	for( i = 0; i < numLegs; i++ )
	{
		...
		animator->GetJointTransform( ankleJoints[i], gameLocal.GetGameTimeMs(), ankleOrigin, ankleAxis[i] );
	...
	animator->GetJointTransform( waistJoint, gameLocal.GetGameTimeMs(), waistOrigin, waistAxis );
	...
	for( i = 0; i < numLegs; i++ )
	{
		...
		animator->GetJointTransform( hipJoints[i], gameLocal.GetGameTimeMs(), hipOrigin, axis );
		hipOrigin = modelOrigin + waistOffset + hipOrigin * modelAxis;
		hipDir = hipForward[i] * axis * modelAxis;

		// get the IK bend direction
		animator->GetJointTransform( kneeJoints[i], gameLocal.GetGameTimeMs(), kneeOrigin, axis );
...
{
	idVec3 shoulderOrigin, elbowOrigin, handOrigin, shoulderDir, elbowDir;
	idMat3 shoulderAxis[MAX_ARMS], elbowAxis[MAX_ARMS], axis;
	trace_t trace;

	idVec3 modelOrigin = self->GetRenderEntity()->origin;
	idMat3 modelAxis = self->GetRenderEntity()->axis;

	// solve IK
	for( int i = 0; i < numArms; i++ )
	{
		// get the position of the shoulder in world space
		animator->GetJointTransform( shoulderJoints[i], gameLocal.GetGameTimeMs(), shoulderOrigin, axis );
		...
		// get the IK bend direction
		animator->GetJointTransform( elbowJoints[i], gameLocal.GetGameTimeMs(), elbowOrigin, axis );
