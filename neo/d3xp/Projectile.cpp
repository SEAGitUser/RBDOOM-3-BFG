...
void idProjectile::Restore( idRestoreGame* savefile )
{
	...
	if( smokeFly != NULL )
	{
		...
		dir.NormalizeFast();
		gameLocal.smokeParticles->EmitSmoke( smokeFly, gameLocal.GetGameTimeMs(), gameLocal.random.RandomFloat(), GetPhysics()->GetOrigin(), GetPhysics()->GetAxis(), timeGroup /*_D3XP*/ );
	}
...
void idProjectile::Launch( const idVec3& start, const idVec3& dir, const idVec3& pushVelocity, const float timeSinceFire, const float launchPower, const float dmgPower )
{
	...
	thrust *= mass;
	thrust_start = SEC2MS( startthrust ) + gameLocal.GetGameTimeMs();
	thrust_end = SEC2MS( endthrust ) + gameLocal.GetGameTimeMs();
	...
	if( *smokeName != '\0' )
	{
		smokeFly = declManager->FindType( DECL_PARTICLE, smokeName )->Cast<idDeclParticle>();
		smokeFlyTime = gameLocal.GetGameTimeMs();
	}
...
void idProjectile::Think()
{
	...
	{
		if( thrust && ( gameLocal.GetGameTimeMs() < thrust_end ) )
		{
			// evaluate force
			thruster.SetForce( GetPhysics()->GetAxis()[ 0 ] * thrust );
			thruster.Evaluate( gameLocal.GetGameTimeMs() );
		}
	}
...
void idProjectile::AddParticlesAndLight()
{
	...
	if( renderLight.lightRadius.x > 0.0f && g_projectileLights.GetBool() )
	{
		...
		if( ( lightDefHandle != -1 ) )
		{
			...
			{
				idVec3 color( 0, 0, 0 );
				if( gameLocal.GetGameTimeMs() < lightEndTime )
				{
...
void idProjectile::Explode( const trace_t& collision, idEntity* ignore )
{
	...
	if( *light_shader )
	{
		...
		lightStartTime = gameLocal.GetGameTimeMs();
		lightEndTime = MSEC_ALIGN_TO_FRAME( gameLocal.GetGameTimeMs() + SEC2MS( light_fadetime ) );
...
{
	float gravity = projectile->GetFloat( "gravity" );
...
void idProjectile::CatchProjectile( idEntity* o, const char* reflectName )
{
	...
	{
		auto proj = static_cast< idGuidedProjectile* >( this );
...
bool idProjectile::ClientReceiveEvent( int event, int time, const idBitMsg& msg )
{
	...
	if( event == EVENT_DAMAGE_EFFECT )
	{
		...
		collision.c.material = ( index != -1 ) ? declManager->DeclByIndex( DECL_MATERIAL, index )->Cast<idMaterial>() : NULL;
...
void idGuidedProjectile::Think()
{
	...
	if( state == LAUNCHED && !unGuided )
	{
		...
		if( rndUpdateTime < gameLocal.GetGameTimeMs() )
		{
			...
			rndUpdateTime = gameLocal.GetGameTimeMs() + 200;
		}
...
void idSoulCubeMissile::KillTarget( const idVec3& dir )
{
	...
	if( enemy.GetEntity() && enemy.GetEntity()->IsType( idActor::Type ) )
	{
		...
		orbitTime = gameLocal.GetGameTimeMs();
		smokeKillTime = 0;
		smokeName = spawnArgs.GetString( "smoke_kill" );
		if( *smokeName != '\0' )
		{
			smokeKill = declManager->FindType( DECL_PARTICLE, smokeName )->Cast<idDeclParticle>();
			smokeKillTime = gameLocal.GetGameTimeMs();
		}
...
void idSoulCubeMissile::Think()
{
	...
	if( state == LAUNCHED )
	{
		if( killPhase )
		{
			...
			if( gameLocal.GetGameTimeMs() < orbitTime + 1500 )
			{
			...
			if( accelTime && gameLocal.GetGameTimeMs() < launchTime + accelTime * 1000 )
			{
				pct = ( gameLocal.GetGameTimeMs() - launchTime ) / ( accelTime * 1000 );
...
void idSoulCubeMissile::Launch( const idVec3& start, const idVec3& dir, const idVec3& pushVelocity, const float timeSinceFire, const float launchPower, float dmgPower )
{
	...
	launchTime = gameLocal.GetGameTimeMs();
	killPhase = false;
	UpdateVisuals();
	...
	if( state == LAUNCHED )
	{
		...
		for( int i = 0; i < beamTargets.Num(); i++ )
		{
			...
			if( !idStr::Cmp( beamEnt->GetEntityDefName(), "monster_boss_d3xp_maledict" ) )
			{
				...
				beamEnt->GetJointWorldTransform( bodyJoint, gameLocal.GetGameTimeMs(), org, temp );
			...
			if( gameLocal.GetGameTimeMs() > nextDamageTime )
			{
				bool bfgVision = true;
		...
		ang.pitch = ( gameLocal.GetGameTimeMs() & 4095 ) * 360.0f / -4096.0f;
		ang.yaw = ang.pitch;
		ang.roll = 0.0f;
		SetAngles( ang );

		ang.pitch = ( gameLocal.GetGameTimeMs() & 2047 ) * 360.0f / -2048.0f;
		ang.yaw = ang.pitch;
		ang.roll = 0.0f;
...
void idBFGProjectile::Launch( const idVec3& start, const idVec3& dir, const idVec3& pushVelocity, const float timeSinceFire, const float power, const float dmgPower )
{
	...
	//physicsObj.SetAngularExtrapolation( extrapolation_t(EXTRAPOLATION_LINEAR|EXTRAPOLATION_NOSTOP), gameLocal.GetGameTimeMs(), 0, physicsObj.GetAxis().ToAngles(), delta, ang_zero );
	...
	// Major hack for end boss.  :(
	auto maledict = static_cast< idAnimatedEntity* >( gameLocal.FindEntity( "monster_boss_d3xp_maledict_1" ) );
	if( maledict )
	{
		...
		auto bodyJoint = maledict->GetAnimator()->GetJointHandle( "Chest1" );
		maledict->GetJointWorldTransform( bodyJoint, gameLocal.GetGameTimeMs(), realPoint, temp );

		float dist = idVec3( realPoint - GetPhysics()->GetOrigin() ).Length();
		if( dist < radius )
		{
			beamTarget_t bt;
			bt.renderEntity.Clear();
...
void idBFGProjectile::Explode( const trace_t& collision, idEntity* ignore )
{
	...
	for( i = 0; i < beamTargets.Num(); i++ )
	{
		...
		if( player )
		{
			...
			{
				player->SetLastHitTime( gameLocal.GetGameTimeMs() );
				player->AddProjectileHits( 1 );
				damageScale *= player->PowerUpModifier( PROJECTILE_DAMAGE );
			}
		}
