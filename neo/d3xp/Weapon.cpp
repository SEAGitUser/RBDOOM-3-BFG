...
void idWeapon::CacheWeapon( const char* weaponName )
{
	...
	auto weaponDef = gameLocal.FindEntityDef( weaponName, false );
	if( !weaponDef )
	{
		return;
	}
	...
	auto brassDefName = weaponDef->dict.GetString( "def_ejectBrass" );
	if( brassDefName[ 0 ] )
	{
		auto brassDef = gameLocal.FindEntityDef( brassDefName, false );
		if( brassDef )
		{
			const char* clipModelName = nullptr;
	...
	auto guiName = weaponDef->dict.GetString( "gui" );
	if( guiName[ 0 ] )
	{
...
void idWeapon::Restore( idRestoreGame* savefile )
{
	...
	auto projectileDef = gameLocal.FindEntityDef( weaponDef->dict.GetString( "def_projectile" ), false );
	if( projectileDef )
	{
	...
	auto brassDef = gameLocal.FindEntityDef( weaponDef->dict.GetString( "def_ejectBrass" ), false );
	if( brassDef )
	{
	...
	for( int i = 0; i < particleCount; i++ )
	{
		...
		{
			newParticle.particle = declManager->FindType( DECL_PARTICLE, particlename, false )->Cast<idDeclParticle>();
		}
		else {
...
void idWeapon::Clear()
{
	...
	auto kv = spawnArgs.MatchPrefix( "snd_" );
	while( kv )
	{
	...
	hideTime = 300;
	hideDistance = -15.0f;
	hideStartTime = gameLocal.GetGameTimeMs() - hideTime;
	hideStart = 0.0f;
	hideEnd = 0.0f;
	hideOffset = 0.0f;
	hide = false;
	...
	for( int i = 0; i < weaponParticles.Num(); i++ )
	{
		auto part = weaponParticles.GetIndex( i );
		if( !part->smoke )
		{
	...
	for( int i = 0; i < weaponLights.Num(); i++ )
	{
		auto light = weaponLights.GetIndex( i );
...
void idWeapon::InitWorldModel( const idDeclEntityDef* def )
{
	...
	if( model[ 0 ] && attach[ 0 ] )
	{
		...
		auto worldModelRenderEntity = ent->GetRenderEntity();
		if( worldModelRenderEntity )
		{
			assert( ( owner->entityNumber + 1 ) < MAX_TYPE( int16 ) );
			...
			assert( ( LIGHTID_VIEW_MUZZLE_FLASH + owner->entityNumber ) < MAX_TYPE( int16 ) );
		}
	}
	else {
		ent->SetModel( "" );
...
void idWeapon::GetWeaponDef( const char* objectname, int ammoinclip )
{
	...
	if( *smokeName != '\0' )
	{
		weaponSmoke = declManager->FindType( DECL_PARTICLE, smokeName )->Cast<idDeclParticle>();
	}
	else
	{
		weaponSmoke = NULL;
	...
	weaponSmokeStartTime = ( continuousSmoke ) ? gameLocal.GetGameTimeMs() : 0;

	smokeName = weaponDef->dict.GetString( "smoke_strike" );
	if( *smokeName != '\0' )
	{
		strikeSmoke = declManager->FindType( DECL_PARTICLE, smokeName )->Cast<idDeclParticle>();
	}
	else {
		strikeSmoke = NULL;
	}
	...
	if( *guiLightShader != '\0' )
	{
		guiLight.material = declManager->FindMaterial( guiLightShader, false );
	...
	auto kv = weaponDef->dict.MatchPrefix( "snd_" );
	while( kv )
	{
	...
	{
		auto projectileDef = gameLocal.FindEntityDef( projectileName, false );
		if( !projectileDef )
		{
	...
	const idMaterial* flashMaterial;
	idVec3			flashTarget;
	idVec3			flashUp;
	idVec3			flashRight;
	float			flashRadius;
	...
	weaponDef->dict.GetString( "mtr_flashShader", "", &material );
	flashMaterial = declManager->FindMaterial( material, false );
	...
	muzzleFlash.Clear();
	muzzleFlash.lightId = LIGHTID_VIEW_MUZZLE_FLASH + owner->entityNumber; assert( ( LIGHTID_VIEW_MUZZLE_FLASH + owner->entityNumber ) < MAX_TYPE( int16 ) );
	muzzleFlash.allowLightInViewID = owner->entityNumber + 1; assert( ( owner->entityNumber + 1 ) < MAX_TYPE( int16 ) );
	...
	muzzleFlash.SetColorParm( flashColor[ 0 ], flashColor[ 1 ], flashColor[ 2 ], 1.0f );
	...
	worldMuzzleFlash.suppressLightInViewID = owner->entityNumber + 1; assert( ( owner->entityNumber + 1 ) < MAX_TYPE( int16 ) );
	worldMuzzleFlash.allowLightInViewID = 0;
	worldMuzzleFlash.lightId = LIGHTID_WORLD_MUZZLE_FLASH + owner->entityNumber; assert( ( LIGHTID_WORLD_MUZZLE_FLASH + owner->entityNumber ) < MAX_TYPE( int16 ) );
	...
	weaponDef->dict.GetString( "mtr_nozzleGlowShader", "", &material );
	nozzleGlowShader = declManager->FindMaterial( material, false );
	...
	auto shader = spawnArgs.GetString( "snd_hum" );
	if( shader && *shader )
	{
	...
	{
		auto pkv = weaponDef->dict.MatchPrefix( "weapon_particle", NULL );
		while( pkv )
		{
			WeaponParticle_t newParticle;
			newParticle.Clear();
			...
			{
				auto emitterDef = gameLocal.FindEntityDef( "func_emitter", false );
		...
		auto lkv = weaponDef->dict.MatchPrefix( "weapon_light", NULL );
		while( lkv )
		{
			WeaponLight_t newLight;
			newLight.Clear();
			...
			idStr material = weaponDef->dict.GetString( va( "%s_shader", name.c_str() ) );
			newLight.light.material = declManager->FindMaterial( material, false );
...
void idWeapon::MuzzleFlashLight()
{
	...
	muzzleFlash.smMaxLod = g_muzzleFlashLightMaxLod.GetInteger();

	// these will be different each fire
	muzzleFlash.shaderParms[ SHADERPARM_TIMEOFFSET ] = -MS2SEC( gameLocal.GetGameTimeMs() );
...
bool idWeapon::UpdateSkin()
{
	...
	auto func = scriptObject.GetFunction( "UpdateSkin" );
	if( !func )
	{
...
void idWeapon::FlashlightOn()
{
	...
	auto func = scriptObject.GetFunction( "TurnOn" );
	if( !func )
	{
...
void idWeapon::FlashlightOff()
{
	...
	auto func = scriptObject.GetFunction( "TurnOff" );
	if( !func )
	{
...
void idWeapon::LowerWeapon()
{
	if( !hide )
	{
		...
		if( gameLocal.GetGameTimeMs() - hideStartTime < hideTime )
		{
			hideStartTime = gameLocal.GetGameTimeMs() - ( hideTime - ( gameLocal.GetGameTimeMs() - hideStartTime ) );
		}
		else {
			hideStartTime = gameLocal.GetGameTimeMs();
		}
		hide = true;
	}
}
...
void idWeapon::BeginAttack()
{
	...
	{
		lastAttack = gameLocal.GetGameTimeMs();
	}
...
void idWeapon::SetState( const char* statename, int blendFrames )
{
	...
	{
		gameLocal.Printf( "%d: weapon state : %s\n", gameLocal.GetGameTimeMs(), statename );
	}
...
void idWeapon::UpdateNozzleFx()
{
	...
	// shader parms
	//
	int la = gameLocal.GetGameTimeMs() - lastAttack + 1;
	float s = 1.0f;
	float l = 0.0f;
	if( la < nozzleFxFade )
	{
	...
	if( nozzleGlowHandle == -1 )
	{
		...
		nozzleGlow.material = nozzleGlowShader;
		nozzleGlow.shaderParms[ SHADERPARM_TIMESCALE ] = 1.0f;
		nozzleGlow.shaderParms[ SHADERPARM_TIMEOFFSET ] = -MS2SEC( gameLocal.GetGameTimeMs() );
...
bool idWeapon::BloodSplat( float size )
{
	...
	auto mtr = declManager->FindMaterial( "textures/decals/duffysplatgun" );
...
void idWeapon::MuzzleRise( idVec3& origin, idMat3& axis )
{
	...
	time = kick_endtime - gameLocal.GetGameTimeMs();
	if( time <= 0 )
	{
		return;
	}
...
void idWeapon::AlertMonsters()
{
	...
	end += muzzleFlash.axis * muzzleFlash.right * idMath::Sin16( MS2SEC( gameLocal.GetGameTimeMs() ) * 31.34f );
	end += muzzleFlash.axis * muzzleFlash.up * idMath::Sin16( MS2SEC( gameLocal.GetGameTimeMs() ) * 12.17f );
	gameLocal.clip.TracePoint( tr, muzzleFlash.origin, end, CONTENTS_OPAQUE | MASK_SHOT_RENDERMODEL | CONTENTS_FLASHLIGHT_TRIGGER, owner );
	if( g_debugWeapon.GetBool() )
	{
		gameRenderWorld->DebugLine( idColor::yellow, muzzleFlash.origin, end, 0 );
...
void idWeapon::PresentWeapon( bool showViewModel )
{
	...
	if( isPlayerFlashlight )
	{
		...
		fraccos = idMath::Cos( ( gameLocal.GetFrameNum() & 255 ) / 127.0f * idMath::PI );

		static unsigned int divisor = 32;
		unsigned int val = ( gameLocal.GetFrameNum() + gameLocal.GetFrameNum() / divisor ) & 255;
		fraccos2 = idMath::Cos( val / 127.0f * idMath::PI );
		...
		if( gameLocal.GetGameTimeMs() - hideStartTime < hideTime )
		{
			float frac = ( float ) ( gameLocal.GetGameTimeMs() - hideStartTime ) / ( float ) hideTime;
	...
	if( worldModel.GetEntity() && worldModel.GetEntity()->GetRenderEntity() )
	{
		...
		if( common->IsMultiplayer() || g_showPlayerShadow.GetBool() || pm_thirdPerson.GetBool() )
		{
			...
			worldModel.GetEntity()->GetRenderEntity()->suppressShadowInViewID = owner->entityNumber + 1; assert( ( owner->entityNumber + 1 ) < MAX_TYPE( int16 ) );
			worldModel.GetEntity()->GetRenderEntity()->suppressShadowInLightID = LIGHTID_VIEW_MUZZLE_FLASH + owner->entityNumber; assert( ( LIGHTID_VIEW_MUZZLE_FLASH + owner->entityNumber ) < MAX_TYPE( int16 ) );
		}
	}
			...
			if( part->active )
			{
				if( part->smoke )
				{
					...
					if( part->emitter != NULL )
					{
						...
						auto rendEnt = part->emitter->GetRenderEntity();
	...
	if( ( !lightOn && ( gameLocal.GetGameTimeMs() >= muzzleFlashEnd ) ) || IsHidden() )
	{
		if( muzzleFlashHandle != -1 )
		{
			gameRenderWorld->FreeLightDef( muzzleFlashHandle );
...
ammo_t idWeapon::GetAmmoNumForName( const char* ammoname )
{
	...
	auto ammoDict = gameLocal.FindEntityDefDict( "ammo_types", false );
	if( ammoDict == NULL )
	{
...
bool idWeapon::ClientReceiveEvent( int event, int time, const idBitMsg& msg )
{
	switch( event )
	{
		case EVENT_RELOAD:
		{
			...
			if( owner != NULL && !owner->IsLocallyControlled() && ( gameLocal.GetGameTimeMs() - time < 1000 ) )
			{
...
{
	auto func = scriptObject.GetFunction( statename );
	if( !func )
	{
		assert( 0 );
		gameLocal.Error( "Can't find function '%s' in object '%s'", statename, scriptObject.GetTypeName() );
...
void idWeapon::Event_PlayAnim( animChannel_t channel, const char* animname )
{
	...
	if( !anim )
	{
		...
		animator.Clear( channel, gameLocal.GetGameTimeMs(), FRAME2MS( animBlendFrames ) );
		animDoneTime = 0;
	}
	else
	{
		...
		if( worldModel.GetEntity() )
		{
			...
			if( anim )
			{
				worldModel.GetEntity()->GetAnimator()->PlayAnim( channel, anim, gameLocal.GetGameTimeMs(), FRAME2MS( animBlendFrames ) );
			}
		}
	}
	animBlendFrames = 0;
...
void idWeapon::Event_PlayCycle( animChannel_t channel, const char* animname )
{
	...
	else
	{
		...
		{
			anim = worldModel.GetEntity()->GetAnimator()->GetAnim( animname );
			worldModel.GetEntity()->GetAnimator()->CycleAnim( channel, anim, gameLocal.GetGameTimeMs(), FRAME2MS( animBlendFrames ) );
		}
	}
	animBlendFrames = 0;
	idThread::ReturnInt( 0 );
...
{
	muzzleFlash.SetColorParm( parm0, parm1, parm2, parm3 );

	worldMuzzleFlash.SetColorParm( parm0, parm1, parm2, parm3 );
...
void idWeapon::Event_LaunchProjectilesEllipse( int num_projectiles, float spreada, float spreadb, float fuseOffset, float power )
{
	...
	// add some to the kick time, incrementally moving repeat firing weapons back
	if( kick_endtime < gameLocal.GetGameTimeMs() )
	{
		kick_endtime = gameLocal.GetGameTimeMs();
	}
	kick_endtime += muzzle_kick_time;
	if( kick_endtime > gameLocal.GetGameTimeMs() + muzzle_kick_maxtime )
	{
		kick_endtime = gameLocal.GetGameTimeMs() + muzzle_kick_maxtime;
	}
	...
	// reset muzzle smoke
	weaponSmokeStartTime = gameLocal.GetGameTimeMs();
}
...
void idWeapon::Event_StartWeaponParticle( const char* name )
{
	...
	{
		part->active = true;
		part->startTime = gameLocal.GetGameTimeMs();
...
void idWeapon::Event_StartWeaponLight( const char* name )
{
	...
	{
		light->active = true;
		light->startTime = gameLocal.GetGameTimeMs();
	}
}
		...
		if( ent != NULL )
		{
			...
			if( weaponDef->dict.GetBool( "impact_damage_effect" ) )
			{
				...
				else
				{
					...
					if( gameLocal.GetGameTimeMs() > nextStrikeFx )
					{
						const char* decal;
						// project decal
						...
						nextStrikeFx = gameLocal.GetGameTimeMs() + 200;
					}
					else
					{
						hitSound = "";
					...
					strikeSmokeStartTime = gameLocal.GetGameTimeMs();
