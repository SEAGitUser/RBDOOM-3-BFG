...
{
	auto & edge = file->GetEdge( edgeNum );
	idVec3 v1 = file->GetVertex( edge.vertexNum[ 0 ] );
	idVec3 v2 = file->GetVertex( edge.vertexNum[ 1 ] );
	float d1 = v1 * plane.Normal() - plane.Dist();
	float d2 = v2 * plane.Normal() - plane.Dist();
...
bool idAASLocal::FloorEdgeSplitPoint( idVec3& bestSplit, int areaNum, const idPlane& pathPlane, const idPlane& frontPlane, bool closest ) const
{
	...
	for( i = 0; i < area->numFaces; i++ )
	{
		...
		face = &file->GetFace( idMath::Abs( faceNum ) );
		...
		for( j = 0; j < face->numEdges; j++ )
		{
			...
			if( !EdgeSplitPoint( split, idMath::Abs( edgeNum ), pathPlane ) )
			{
				continue;
			}
	...
	return ( closest )? ( bestDist < maxWalkPathDistance ) : ( bestDist > -0.1f );
}
...
int idAASLocal::GetWallEdges( int areaNum, const idBounds& bounds, int travelFlags, int* edges, int maxEdges ) const
{
	...
	for( curArea = areaNum; queueStart < queueEnd; curArea = areaQueue[ ++queueStart ] )
	{
		...
		for( i = 0; i < area->numFaces; i++ )
		{
			...
			face1 = &file->GetFace( idMath::Abs( face1Num ) );
			...
			for( j = 0; j < face1->numEdges; j++ )
			{
				...
				absEdge1Num = idMath::Abs( edge1Num );
				...
				for( k = 0; k < area->numFaces; k++ )
				{
					...
					face2 = &file->GetFace( idMath::Abs( face2Num ) );
					...
					{
						edge2Num = idMath::Abs( file->GetEdgeIndex( face2->firstEdge + l ) );
