...
bool idAASFindCover::TestArea( const idAAS* aas, int areaNum )
{
	...
	int numPVSAreas = gameLocal.pvs.GetPVSAreas( idBounds( areaCenter ).Expand( 16.0f ), PVSAreas, idEntity::MAX_PVS_AREAS );
...
void idAI::Spawn()
{
	...
	{
		af.SetupPose( this, gameLocal.GetGameTimeMs() );
...
void idAI::InitMuzzleFlash()
{
	...
	worldMuzzleFlash.material = declManager->FindMaterial( shader, false );
...
void idAI::DormantEnd()
{
	...
	if( particles.Num() )
	{
		for( int i = 0; i < particles.Num(); i++ )
		{
			particles[i].time = gameLocal.GetGameTimeMs();
		}
	}
...
void idAI::Think()
{
	...
	if( !aas && developer.GetBool() && !fl.hidden && !num_cinematics ) {
		gameRenderWorld->DrawText( "No AAS", physicsObj.GetAbsBounds().GetCenter(), 0.1f, idColor::white, gameLocal.GetLocalPlayer()->viewAngles.ToMat3(), 1, 1 );
	}
	...
	if( ai_showHealth.GetBool() )
	{
		...
		gameRenderWorld->DrawText( va( "%d", ( int )health ), this->GetEyePosition() + aboveHead, 0.5f, idColor::white, gameLocal.GetLocalPlayer()->viewAngles.ToMat3() );
	}
}
...
void idAI::SetAAS()
{
	...
	if( aas )
	{
		auto settings = aas->GetSettings();
		if( settings )
		{
			if( !settings->ValidForBounds( physicsObj.GetBounds() ) )
			{
...
float idAI::TravelDistance( const idVec3& start, const idVec3& end ) const
{
	...
	if( !aas )
	{
		...
		{
			gameRenderWorld->DebugLine( idColor::blue, start, end, 1, false );
			gameRenderWorld->DrawText( va( "%d", ( int )dist ), ( start + end ) * 0.5f, 0.1f, idColor::white, gameLocal.GetLocalPlayer()->viewAngles.ToMat3() );
		}
...
void idAI::StopMove( moveStatus_t status )
{
	...
	AI_BLOCKED			= false;
	move.startTime		= gameLocal.GetGameTimeMs();
	move.duration		= 0;
	move.range			= 0.0f;
	move.speed			= 0.0f;
	move.anim			= 0;
	...
	move.lastMoveTime	= gameLocal.GetGameTimeMs();
}
...
bool idAI::NewWanderDir( const idVec3& dest )
{
	...
	move.nextWanderTime = gameLocal.GetGameTimeMs() + ( gameLocal.random.RandomFloat() * 500 + 500 );
...
bool idAI::GetMovePos( idVec3& seekPos )
{
	...
	move.moveStatus = MOVE_STATUS_MOVING;
	bool result = false;
	if( gameLocal.GetGameTimeMs() > move.blockTime )
	{
	...
	{
		// wander around
		if( ( gameLocal.GetGameTimeMs() > move.nextWanderTime ) || !StepDirection( move.wanderYaw ) )
		{
		...
		if( ai_debugMove.GetBool() )
		{
			gameRenderWorld->DebugLine( idColor::yellow, org, seekPos, 1, true );
		}
	}
	else
	{
	...
	{
		gameRenderWorld->DebugLine( idColor::cyan, physicsObj.GetOrigin(), seekPos );
	}
...
void idAI::BlockedFailSafe()
{
	...
	if( move.lastMoveTime < gameLocal.GetGameTimeMs() - blockedMoveTime )
	{
		if( lastAttackTime < gameLocal.GetGameTimeMs() - blockedAttackTime )
		{
			AI_BLOCKED = true;
			move.lastMoveTime = gameLocal.GetGameTimeMs();
		}
...
void idAI::Turn()
{
	...
	if( anim_turn_angles && animflags.anim_turn )
	{
		...
		animator.GetDeltaRotation( 0, gameLocal.GetGameTimeMs(), rotateAxis );
		current_yaw = idMath::AngleNormalize180( anim_turn_yaw + rotateAxis[ 0 ].ToYaw() );
	}
	else {
		diff = idMath::AngleNormalize180( ideal_yaw - current_yaw );
		turnVel += AI_TURN_SCALE * diff * MS2SEC( gameLocal.GetGameTimeMs() - gameLocal.GetPreviousGameTimeMs() );
		if( turnVel > turnRate )
		{
			turnVel = turnRate;
		}
		...
		turnAmount = turnVel * MS2SEC( gameLocal.GetGameTimeMs() - gameLocal.GetPreviousGameTimeMs() );
		if( ( diff >= 0.0f ) && ( turnAmount >= diff ) )
		{
			turnVel = diff / MS2SEC( gameLocal.GetGameTimeMs() - gameLocal.GetPreviousGameTimeMs() );
			turnAmount = diff;
		}
	...
	{
		const idVec3& org = physicsObj.GetOrigin();
		gameRenderWorld->DebugLine( idColor::red, org, org + idAngles( 0, ideal_yaw, 0 ).ToForward() * 64, 1 );
		gameRenderWorld->DebugLine( idColor::green, org, org + idAngles( 0, current_yaw, 0 ).ToForward() * 48, 1 );
		gameRenderWorld->DebugLine( idColor::yellow, org, org + idAngles( 0, current_yaw + turnVel, 0 ).ToForward() * 32, 1 );
	}
}
...
void idAI::GetMoveDelta( const idMat3& oldaxis, const idMat3& axis, idVec3& delta )
{
	...
	animator.GetDelta( gameLocal.GetPreviousGameTimeMs(), gameLocal.GetGameTimeMs(), delta );
	delta = axis * delta;
...
void idAI::CheckObstacleAvoidance( const idVec3& goalPos, idVec3& newPos )
{
	...
	{
		gameRenderWorld->DebugLine( idColor::blue, goalPos + idVec3( 1.0f, 1.0f, 0.0f ), goalPos + idVec3( 1.0f, 1.0f, 64.0f ), 1 );
		gameRenderWorld->DebugLine( foundPath ? idColor::yellow : idColor::red, path.seekPos, path.seekPos + idVec3( 0.0f, 0.0f, 64.0f ), 1 );
	}
	if( !foundPath )
	{
...
void idAI::AnimMove()
{
	...
	{
		if( gameLocal.GetGameTimeMs() < move.startTime + move.duration )
		{
			goalPos = move.moveDest - move.moveDir * MS2SEC( move.startTime + move.duration - gameLocal.GetGameTimeMs() );
	...
	if( ai_debugMove.GetBool() )
	{
		gameRenderWorld->DebugLine( idColor::cyan, oldorigin, physicsObj.GetOrigin(), 5000 );
	}
	...
	if( ai_debugMove.GetBool() )
	{
		gameRenderWorld->DebugBounds( idColor::magenta, physicsObj.GetBounds(), org, 1 );
		gameRenderWorld->DebugBounds( idColor::magenta, physicsObj.GetBounds(), move.moveDest, 1 );
		gameRenderWorld->DebugLine( idColor::yellow, org + EyeOffset(), org + EyeOffset() + viewAxis[ 0 ] * physicsObj.GetGravityAxis() * 16.0f, 1, true );
		DrawRoute();
	}
}
...
idVec3 Seek( idVec3& vel, const idVec3& org, const idVec3& goal, float prediction )
{
	...
	idVec3 seekVel = goalDelta * MS2SEC( gameLocal.GetGameTimeMs() - gameLocal.GetPreviousGameTimeMs() );
	return seekVel;
}
...
void idAI::SlideMove()
{
	...
	vel -= vel * AI_FLY_DAMPENING * MS2SEC( gameLocal.GetGameTimeMs() - gameLocal.GetPreviousGameTimeMs() );
	vel += goalDelta * MS2SEC( gameLocal.GetGameTimeMs() - gameLocal.GetPreviousGameTimeMs() );
...
void idAI::AddFlyBob( idVec3& vel )
{
	...
	if( fly_bob_strength )
	{
		t = MS2SEC( gameLocal.GetGameTimeMs() + entityNumber * 497 );
		fly_bob_add = ( viewAxis[ 1 ] * idMath::Sin16( t * fly_bob_horz ) + viewAxis[ 2 ] * idMath::Sin16( t * fly_bob_vert ) ) * fly_bob_strength;
		vel += fly_bob_add * MS2SEC( gameLocal.GetGameTimeMs() - gameLocal.GetPreviousGameTimeMs() );
		if( ai_debugMove.GetBool() )
		{
			const idVec3& origin = physicsObj.GetOrigin();
			gameRenderWorld->DebugArrow( idColor::orange, origin, origin + fly_bob_add, 0 );
		}
	}
}
...
void idAI::AdjustFlyHeight( idVec3& vel, const idVec3& goalPos )
{
	...
	if( origin.z > goalPos.z )
	{
		...
		if( ai_debugMove.GetBool() )
		{
			gameRenderWorld->DebugBounds( goLower ? idColor::red : idColor::green, physicsObj.GetBounds(), path.endPos, 1 );
		}
	}
...
void idAI::AdjustFlySpeed( idVec3& vel )
{
	...
	speed = vel.Normalize();
	speed += ( move.speed - speed ) * MS2SEC( gameLocal.GetGameTimeMs() - gameLocal.GetPreviousGameTimeMs() );
	if( speed < 0.0f )
	{
		speed = 0.0f;
	}
...
void idAI::FlyMove()
{
	...
	if( ai_debugMove.GetBool() )
	{
		gameLocal.Printf( "%d: %s: %s, vel = %.2f, sp = %.2f, maxsp = %.2f\n", gameLocal.GetGameTimeMs(), name.c_str(), moveCommandString[ move.moveCommand ], physicsObj.GetLinearVelocity().Length(), move.speed, fly_speed );
	}
	...
	else
	{
		...
		{
			move.blockTime = gameLocal.GetGameTimeMs() + 500;
			AI_BLOCKED = true;
		}
	}
	...
	if( ai_debugMove.GetBool() )
	{
		gameRenderWorld->DebugLine( idColor::cyan, oldorigin, physicsObj.GetOrigin(), 4000 );
		gameRenderWorld->DebugBounds( idColor::orange, physicsObj.GetBounds(), org, 1 );
		gameRenderWorld->DebugBounds( idColor::magenta, physicsObj.GetBounds(), move.moveDest, 1 );
		gameRenderWorld->DebugLine( idColor::red, org, org + physicsObj.GetLinearVelocity(), 1, true );
		gameRenderWorld->DebugLine( idColor::blue, org, goalPos, 1, true );
		gameRenderWorld->DebugLine( idColor::yellow, org + EyeOffset(), org + EyeOffset() + viewAxis[ 0 ] * physicsObj.GetGravityAxis() * 16.0f, 1, true );
		DrawRoute();
	}
}
...
void idAI::StaticMove()
{
	...
	if( ai_debugMove.GetBool() )
	{
		const idVec3& org = physicsObj.GetOrigin();
		gameRenderWorld->DebugBounds( idColor::magenta, physicsObj.GetBounds(), org, 1 );
		gameRenderWorld->DebugLine( idColor::blue, org, move.moveDest, 1, true );
		gameRenderWorld->DebugLine( idColor::yellow, org + EyeOffset(), org + EyeOffset() + viewAxis[ 0 ] * physicsObj.GetGravityAxis() * 16.0f, 1, true );
	}
}
...
const idDeclParticle* idAI::SpawnParticlesOnJoint( particleEmitter_t& pe, const char* particleName, const char* jointName )
{
	...
	{
		animator.GetJointTransform( pe.joint, gameLocal.GetGameTimeMs(), origin, axis );
		origin = renderEntity.origin + origin * renderEntity.axis;

		BecomeActive( TH_UPDATEPARTICLES );
		if( !gameLocal.GetGameTimeMs() )
		{
		...
		{
			pe.time = gameLocal.GetGameTimeMs();
		}
...
void idAI::Killed( idEntity* inflictor, idEntity* attacker, int damage, const idVec3& dir, int location )
{
	...
	if( head.GetEntity() )
	{
		...
		head.GetEntity()->GetAnimator()->ClearAllAnims( gameLocal.GetGameTimeMs(), 100 );
	}
...
void idAI::UpdateEnemyPosition()
{
	...
	if( ai_debugMove.GetBool() )
	{
		gameRenderWorld->DebugBounds( idColor::ltGrey, enemyEnt->GetPhysics()->GetBounds(), lastReachableEnemyPos, 1 );
		gameRenderWorld->DebugBounds( idColor::white, enemyEnt->GetPhysics()->GetBounds(), lastVisibleReachableEnemyPos, 1 );
	}
}
...
idProjectile* idAI::LaunchProjectile( const char* jointname, idEntity* target, bool clampToAttackCone )
{
	...
	float t = MS2SEC( gameLocal.GetGameTimeMs() + entityNumber * 497 );
...
bool idAI::TestMelee() const
{
	...
	if( ai_debugMove.GetBool() )
	{
		gameRenderWorld->DebugBounds( idColor::yellow, bounds, vec3_zero, 1 );
	}
...
bool idAI::AttackMelee( const char* meleeDefName )
{
	...
	if( enemyEnt->IsType( idPlayer::Type ) && g_skill.GetInteger() < 2 )
	{
		...
		{
			int	t = gameLocal.GetGameTimeMs() - player->lastSavingThrowTime;
			if( t > SAVING_THROW_TIME )
			{
				player->lastSavingThrowTime = gameLocal.GetGameTimeMs();
				t = 0;
			}
			if( t < 1000 )
			{
...
void idAI::PushWithAF()
{
	...
	af.ChangePose( this, gameLocal.GetGameTimeMs() );
...
void idAI::GetMuzzle( const char* jointname, idVec3& muzzle, idMat3& axis )
{
	...
	else
	{
		...
		GetJointWorldTransform( joint, gameLocal.GetGameTimeMs(), muzzle, axis );
	}
}
...
void idAI::TriggerWeaponEffects( const idVec3& muzzle )
{
	...
	if( flashJointWorld != INVALID_JOINT )
	{
		...
		GetJointWorldTransform( flashJointWorld, gameLocal.GetGameTimeMs(), org, axis );

		if( worldMuzzleFlash.lightRadius.x > 0.0f )
		{
			worldMuzzleFlash.smMaxLod = g_projectileLightMaxLod.GetInteger();

			worldMuzzleFlash.axis = axis;
			worldMuzzleFlash.shaderParms[SHADERPARM_TIMEOFFSET] = -MS2SEC( gameLocal.GetGameTimeMs() );
			if( worldMuzzleFlashHandle != - 1 )
			{
			...
			muzzleFlashEnd = gameLocal.GetGameTimeMs() + flashTime;
			UpdateVisuals();
		}
	}
}
...
void idAI::UpdateMuzzleFlash()
{
	if( worldMuzzleFlashHandle != -1 )
	{
		if( gameLocal.GetGameTimeMs() >= muzzleFlashEnd )
		{
			gameRenderWorld->FreeLightDef( worldMuzzleFlashHandle );
			worldMuzzleFlashHandle = -1;
		}
			...
			idVec3 muzzle;
			animator.GetJointTransform( flashJointWorld, gameLocal.GetGameTimeMs(), muzzle, worldMuzzleFlash.axis );
			animator.GetJointTransform( flashJointWorld, gameLocal.GetGameTimeMs(), muzzle, worldMuzzleFlash.axis );
...
void idAI::SetChatSound()
{
	...
	if( snd != NULL && *snd != '\0' )
	{
		...
		chat_time = gameLocal.GetGameTimeMs() + chat_min + gameLocal.random.RandomFloat() * ( chat_max - chat_min );
	}
	else
	{
		chat_snd = NULL;
...
void idAI::PlayChatter()
{
	...
	if( AI_DEAD || !chat_snd || ( chat_time > gameLocal.GetGameTimeMs() ) )
	{
		return;
	}
	...
	if( ( thinkFlags & TH_UPDATEPARTICLES ) && !IsHidden() )
	{
		...
		for( int i = 0; i < particles.Num(); i++ )
		{
			...
			if( particles[i].particle && particles[i].time )
			{
				...
				{
					animator.GetJointTransform( particles[i].joint, gameLocal.GetGameTimeMs(), realVector, realAxis );
...
void idAI::TriggerFX( const char* joint, const char* fx )
{
	if( !idStr::Cmp( joint, "origin" ) )
	{
		...
		GetJointWorldTransform( jointNum, gameLocal.GetGameTimeMs(), joint_origin, joint_axis );
...
idEntity* idAI::StartEmitter( const char* name, const char* joint, const char* particle )
{
	...
	GetJointWorldTransform( jointNum, gameLocal.GetGameTimeMs(), offset, axis );

	/*animator.GetJointTransform( jointNum, gameLocal.GetGameTimeMs(), offset, axis );
...
bool idAI::UpdateAnimationControllers()
{
	...
	{
		GetJointWorldTransform( orientationJoint, gameLocal.GetGameTimeMs(), orientationJointPos, orientationJointAxis );
	...
	if( focusJoint != INVALID_JOINT )
	{
		if( headEnt )
		{
			headEnt->GetJointWorldTransform( focusJoint, gameLocal.GetGameTimeMs(), eyepos, axis );
		}
		else
		{
			GetJointWorldTransform( focusJoint, gameLocal.GetGameTimeMs(), eyepos, axis );
		}
		eyeOffset.z = eyepos.z - physicsObj.GetOrigin().z;
		if( ai_debugMove.GetBool() )
		{
			gameRenderWorld->DebugLine( idColor::red, eyepos, eyepos + orientationJointAxis[ 0 ] * 32.0f, 1 );
		}
	}
	else
	{
	...
	if( !allowJointMod || !allowEyeFocus || ( gameLocal.GetGameTimeMs() >= focusTime ) )
	{
	...
	if( eyeAng != diff )
	{
		...
		{
			alignHeadTime = gameLocal.GetGameTimeMs() + ( 0.5f + 0.5f * gameLocal.random.RandomFloat() ) * focusAlignTime;
		}
	}
	...
	if( ( gameLocal.GetGameTimeMs() >= alignHeadTime ) || ( gameLocal.GetGameTimeMs() < forceAlignHeadTime ) )
	{
		alignHeadTime = gameLocal.GetGameTimeMs() + ( 0.5f + 0.5f * gameLocal.random.RandomFloat() ) * focusAlignTime;
