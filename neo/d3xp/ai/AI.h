...
public:
	...
	virtual bool		TestArea( const idAAS*, int areaNum );
...
public:
	...
	void					Spawn();
	void					HeardSound( idEntity*, const char* action );
	...
	static bool				PredictPath( const idEntity*, const idAAS*, const idVec3& start, const idVec3& velocity, int totalTime, int frameTime, int stopEvent, predictedPath_t& path );
...
protected:
	...
	// movement
	virtual void			ApplyImpulse( idEntity*, int id, const idVec3& point, const idVec3& impulse );
	...
	bool					PathToGoal( aasPath_t&, int areaNum, const idVec3& origin, int goalAreaNum, const idVec3& goalOrigin ) const;
	void					DrawRoute() const;
	bool					GetMovePos( idVec3& seekPos );
	bool					MoveDone() const;
	bool					EntityCanSeePos( idActor*, const idVec3& actorOrigin, const idVec3& pos );
	void					BlockedFailSafe();

	// movement control
	void					StopMove( moveStatus_t );
	bool					FaceEnemy();
	bool					FaceEntity( idEntity* );
	bool					DirectMoveToPosition( const idVec3& pos );
	bool					MoveToEnemyHeight();
	bool					MoveOutOfRange( idEntity*, float range );
	bool					MoveToAttackPosition( idEntity*, int attack_anim );
	bool					MoveToEnemy();
	bool					MoveToEntity( idEntity* );
	bool					MoveToPosition( const idVec3& pos );
	bool					MoveToCover( idEntity*, const idVec3& pos );
	...
	// effects
	const idDeclParticle*	SpawnParticlesOnJoint( particleEmitter_t&, const char* particleName, const char* jointName );
	...
	void					Event_TurnToEntity( idEntity* );
	void					Event_MoveStatus();
	void					Event_StopMove();
	void					Event_MoveToCover();
	void					Event_MoveToEnemy();
	void					Event_MoveToEnemyHeight();
	void					Event_MoveOutOfRange( idEntity*, float range );
	void					Event_MoveToAttackPosition( idEntity*, const char* attack_anim );
	void					Event_MoveToEntity( idEntity* );
	...
	void					Event_FaceEntity( idEntity* );
	void					Event_WaitAction( const char* waitForState );
	void					Event_GetCombatNode();
	void					Event_EnemyInCombatCone( idEntity*, int use_current_enemy_location );
	void					Event_WaitMove();
	void					Event_GetJumpVelocity( const idVec3& pos, float speed, float max_height );
	void					Event_EntityInAttackCone( idEntity* );
	void					Event_CanSeeEntity( idEntity* );
	...
	void					Event_ThrowAF();
	void					Event_SetAngles( idAngles const& );
	...
	void 					Event_CanReachEntity( idEntity* );
	void					Event_CanReachEnemy();
	void					Event_GetReachableEntityPosition( idEntity* );
