...
bool LineIntersectsPath( const idVec2& start, const idVec2& end, const pathNode_t* node )
{
	...
	idWinding2D::Plane2DFromPoints( plane1, start, end );
	float d0 = plane1.x * node->pos.x + plane1.y * node->pos.y + plane1.z;
	while( node->parent )
	{
		d1 = plane1.x * node->parent->pos.x + plane1.y * node->parent->pos.y + plane1.z;
		if( IEEE_FLT_SIGNBITSET( d0 ) ^ IEEE_FLT_SIGNBITSET( d1 ) )
		{
			idWinding2D::Plane2DFromPoints( plane2, node->pos, node->parent->pos );
...
int PointInsideObstacle( const obstacle_t* obstacles, const int numObstacles, const idVec2& point )
{
	...
	{
		const auto & bounds = obstacles[ i ].bounds;
...
============
*/
void GetPointOutsideObstacles( const obstacle_t* obstacles, const int numObstacles, 
							   idVec2& point, int* obstacle, int* edgeNum )
{
	...
	{
		idWinding2D::Plane2DFromPoints( plane, w[ ( i + 1 ) % w.GetNumPoints() ], w[ i ], true );
	...
	for( i = queue[ 0 ]; queueStart < queueEnd; i = queue[ ++queueStart ] )
	{
		...
		for( j = 0; j < numObstacles; j++ )
		{
			...
			if( !obstacles[ j ].bounds.IntersectsBounds( obstacles[ i ].bounds ) )
				continue;
...
bool GetFirstBlockingObstacle( const obstacle_t* obstacles, int numObstacles, 
							   int skipObstacle, const idVec2& startPos, const idVec2& delta, float& blockingScale, int& blockingObstacle, int& blockingEdgeNum )
{
	int i, edgeNums[ 2 ];
	...
	for( i = 0; i < numObstacles; ++i )
	{
		...
		if( bounds.IntersectsBounds( obstacles[ i ].bounds ) )
		{
...
int GetObstacles( const idPhysics* physics, const idAAS* aas, const idEntity* ignore, int areaNum, const idVec3& startPos, const idVec3& seekPos, obstacle_t* obstacles, int maxObstacles, idBounds& clipBounds )
{
	...
	for( i = 0; i < numListedClipModels && numObstacles < MAX_OBSTACLES; i++ )
	{
		...
		auto & obstacle = obstacles[ numObstacles++ ];
...
/*
============
idObstacleAvoidance::OptimizePath
============
*/
...
{
	int numObstacles, areaNum, startInsideObstacle, destInsideObstacle;
	...
	GetPointOutsideObstacles( obstacles, numObstacles, path.startPosOutsideObstacles.ToVec2(), &startInsideObstacle, NULL );
	if( startInsideObstacle != -1 )
	{
		path.startPosObstacle = obstacles[ startInsideObstacle ].entity;
	}

	// get a goal position outside the obstacles
	GetPointOutsideObstacles( obstacles, numObstacles, path.seekPosOutsideObstacles.ToVec2(), &destInsideObstacle, NULL );
	if( destInsideObstacle != -1 )
	{
		path.seekPosObstacle = obstacles[ destInsideObstacle ].entity;
	}
...
============
*/
bool PathTrace( const idEntity* ent, const idAAS* aas, const idVec3& start, const idVec3& end, int stopEvent, struct pathTrace_t& trace, predictedPath_t& path )
{
	trace_t clipTrace;
	aasTrace_t aasTrace;
...
{
	float x = ( end.ToVec2() - start.ToVec2() ).Length();
	float y = end[ 2 ] - start[ 2 ];

	float a = 4.0f * y * y + 4.0f * x * x;
	float b = -4.0f * speed * speed - 4.0f * y * gravity;
	float c = gravity * gravity;
...
static float HeightForTrajectory( const idVec3& start, float zVel, float gravity )
{
	...
	return start.z - 0.5f * gravity * idMath::Square( zVel / gravity );
}
#endif
...
bool idAI::TestTrajectory( const idVec3& start, const idVec3& end, float zVel, float gravity, float time, float max_height, const idClipModel* clip, int clipmask, const idEntity* ignore, const idEntity* targetEntity, int drawtime )
{
	...
	// maximum height of projectile
	float maxHeight = start.z - 0.5f * gravity * idMath::Square( zVel / gravity );
	// time it takes to fall from the top to the end height
	float t = idMath::Sqrt( ( maxHeight - end.z ) / ( 0.5f * -gravity ) );
	...
	if( t < time )
	{
		...
		points[ 1 ].z = start.z + t2 * zVel + 0.5f * gravity * idMath::Square( t2 );
		// top of parabolic
		t2 = time - t;
		points[ 2 ].ToVec2() = start.ToVec2() + ( end.ToVec2() - start.ToVec2() ) * ( t2 / time );
		points[ 2 ].z = start.z + t2 * zVel + 0.5f * gravity * idMath::Square( t2 );
		// point in the middel between top and end
		t2 = time - t * 0.5f;
		points[ 3 ].ToVec2() = start.ToVec2() + ( end.ToVec2() - start.ToVec2() ) * ( t2 / time );
		points[ 3 ].z = start.z + t2 * zVel + 0.5f * gravity * idMath::Square( t2 );
	}
	else {
		numSegments = 2;
