...
void idSaveGame::WriteDecls()
{
	...
	for( int t = 0; t < declManager->GetNumDeclTypes(); ++t )
	{
		...
		{
			auto decl = declManager->DeclByIndex( ( declType_t )t, d, false );
...
================
*/
void idSaveGame::WriteRenderEntity( const renderEntityParms_t& renderEntity )
{
	int i;
	...
	WriteInt( (int)renderEntity.suppressSurfaceInViewID );
	WriteInt( (int)renderEntity.suppressShadowInViewID );
	WriteInt( (int)renderEntity.suppressShadowInLightID );
	WriteInt( (int)renderEntity.allowSurfaceInViewID );
...
================
*/
void idSaveGame::WriteRenderLight( const renderLightParms_t& renderLight )
{
	WriteMat3( renderLight.axis );
	WriteVec3( renderLight.origin );

	WriteInt( (int)renderLight.suppressLightInViewID );
	WriteInt( (int)renderLight.allowLightInViewID );
	...
	WriteInt( (int)renderLight.lightId );
...
{
	idStr declName;
	//SEA: Temp Fix ( adding new types breaks old savefile loading )
	for( int t = 0; t < 15/*declManager->GetNumDeclTypes()*/; t++ )
	{
		while( true )
		{
			ReadString( declName );
...
================
*/
void idRestoreGame::ReadRenderEntity( renderEntityParms_t& renderEntity )
{
	int i;
	int index;
	...
	ReadInt( i ); renderEntity.suppressSurfaceInViewID = i; assert( i < MAX_TYPE( int16 ) );
	ReadInt( i ); renderEntity.suppressShadowInViewID = i; assert( i < MAX_TYPE( int16 ) );
	ReadInt( i ); renderEntity.suppressShadowInLightID = i; assert( i < MAX_TYPE( int16 ) );
	ReadInt( i ); renderEntity.allowSurfaceInViewID = i; assert( i < MAX_TYPE( int16 ) );
...
================
*/
void idRestoreGame::ReadRenderLight( renderLightParms_t& renderLight )
{
	int index, i;
	...
	ReadInt( i ); renderLight.suppressLightInViewID = i; assert( i < MAX_TYPE( int16 ) );
	ReadInt( i ); renderLight.allowLightInViewID = i; assert( i < MAX_TYPE( int16 ) );
	...
	ReadInt( i ); renderLight.lightId = i;
