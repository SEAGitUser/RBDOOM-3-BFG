...
void idForce_Drag::Evaluate( int time )
{
	...
	physics->SetAngularVelocity( rotation.ToAngularVelocity() / MS2SEC( gameLocal.GetGameTimeMs() - gameLocal.GetPreviousGameTimeMs() ), id );
	
	velocity = physics->GetLinearVelocity( id ) * damping + dir1 * ( ( l1 - l2 ) * ( 1.0f - damping ) / MS2SEC( gameLocal.GetGameTimeMs() - gameLocal.GetPreviousGameTimeMs() ) );
