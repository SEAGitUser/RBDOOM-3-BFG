...
idPhysics_Parametric::idPhysics_Parametric()
{
	...
	current.time = gameLocal.GetGameTimeMs();
	current.atRest = -1;
...
{
	SetLinearExtrapolation( extrapolation_t( EXTRAPOLATION_LINEAR | EXTRAPOLATION_NOSTOP ), gameLocal.GetGameTimeMs(), 0, current.origin, newLinearVelocity, vec3_origin );
	current.linearInterpolation.Init( 0, 0, 0, 0, vec3_zero, vec3_zero );
	Activate();
}
...
void idPhysics_Parametric::SetAngularVelocity( const idVec3& newAngularVelocity, int id )
{
	...
	SetAngularExtrapolation( extrapolation_t( EXTRAPOLATION_LINEAR | EXTRAPOLATION_NOSTOP ), gameLocal.GetGameTimeMs(), 0, current.angles, rotation.ToAngles(), ang_zero );
	current.angularInterpolation.Init( 0, 0, 0, 0, ang_zero, ang_zero );
	Activate();
}
...
const idVec3& idPhysics_Parametric::GetLinearVelocity( int id ) const
{
	...
	curLinearVelocity = current.linearExtrapolation.GetCurrentSpeed( gameLocal.GetGameTimeMs() );
...
const idVec3& idPhysics_Parametric::GetAngularVelocity( int id ) const
{
	...
	angles = current.angularExtrapolation.GetCurrentSpeed( gameLocal.GetGameTimeMs() );
