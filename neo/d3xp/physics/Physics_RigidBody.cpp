...
void idPhysics_RigidBody::DebugDraw()
{
	...
	{
		gameRenderWorld->DrawText( va( "\n%1.2f", mass ), current.i.position, 0.08f, idColor::cyan, gameLocal.GetLocalPlayer()->viewAngles.ToMat3(), 1 );
	}
	...
	if( rb_showInertia.GetBool() )
	{
		...
		gameRenderWorld->DrawText( va( "\n\n\n( %.1f %.1f %.1f )\n( %.1f %.1f %.1f )\n( %.1f %.1f %.1f )",
									   ...
									   I[1].x, I[1].y, I[1].z,
									   I[2].x, I[2].y, I[2].z ),
								   current.i.position, 0.05f, idColor::cyan, gameLocal.GetLocalPlayer()->viewAngles.ToMat3(), 1 );
	}
