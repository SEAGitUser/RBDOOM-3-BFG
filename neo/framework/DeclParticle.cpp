...
void idDeclParticle::ParseParametric( idLexer& src, idParticleParm* parm )
{
	...
	{
		// table
		parm->table = declManager->FindType( DECL_TABLE, token, false )->Cast<idDeclTable>();
	}
}
...
idParticleStage* idDeclParticle::ParseParticleStage( idLexer& src )
{
	...
	while( 1 )
	{
		...
		if( !token.Icmp( "distribution" ) )
		{
			...
			ParseParms( src, stage->distributionParms, _countof( stage->distributionParms ) );
			continue;
		}
		...
		if( !token.Icmp( "direction" ) )
		{
			...
			ParseParms( src, stage->directionParms, _countof( stage->directionParms ) );
			continue;
		}
		...
		if( !token.Icmp( "orientation" ) )
		{
			...
			ParseParms( src, stage->orientationParms, _countof( stage->orientationParms ) );
			continue;
		}
		...
		if( !token.Icmp( "customPath" ) )
		{
			...
			ParseParms( src, stage->customPathParms, _countof( stage->customPathParms ) );
			continue;
		}
...
bool idDeclParticle::LoadBinary( idFile* file, unsigned int checksum )
{
	...
	auto LoadParticleParm = []( idFile* file, idParticleParm& parm )
	{
		idStr name;
		...
		if( name.IsEmpty() )
		{
			...
			parm.table = declManager->FindType( DECL_TABLE, name, false )->Cast<idDeclTable>();
		}
	...
	for( int i = 0; i < numStages; ++i )
	{
		...
		file->ReadBigArray( s->distributionParms, _countof( s->distributionParms ) );
		file->ReadBig( s->directionType );
		file->ReadBigArray( s->directionParms, _countof( s->directionParms ) );
		LoadParticleParm( file, s->speed );
		...
		file->ReadBigArray( s->customPathParms, _countof( s->customPathParms ) );
		file->ReadVec3( s->offset );
		file->ReadBig( s->animationFrames );
		file->ReadFloat( s->animationRate );
		file->ReadFloat( s->initialAngle );
		LoadParticleParm( file, s->rotationSpeed );
		file->ReadBig( s->orientation );
		file->ReadBigArray( s->orientationParms, _countof( s->orientationParms ) );
		LoadParticleParm( file, s->size );
		LoadParticleParm( file, s->aspect );
...
void idDeclParticle::WriteBinary( idFile* file, unsigned int checksum )
{
	...
	auto WriteParticleParm = []( idFile* file, idParticleParm& parm )
	{
	...
	for( int i = 0; i < stages.Num(); i++ )
	{
		...
		file->WriteBigArray( s->distributionParms, _countof( s->distributionParms ) );
		file->WriteBig( s->directionType );
		file->WriteBigArray( s->directionParms, _countof( s->directionParms ) );
		WriteParticleParm( file, s->speed );
		...
		file->WriteBigArray( s->customPathParms, _countof( s->customPathParms ) );
		file->WriteVec3( s->offset );
		file->WriteBig( s->animationFrames );
		file->WriteFloat( s->animationRate );
		file->WriteFloat( s->initialAngle );
		WriteParticleParm( file, s->rotationSpeed );
		file->WriteBig( s->orientation );
		file->WriteBigArray( s->orientationParms, _countof( s->orientationParms ) );
		WriteParticleParm( file, s->size );
		WriteParticleParm( file, s->aspect );
			...
			case PDIST_CYLINDER:  	// ( sizeX sizeY sizeZ ringFraction )
			{
				...
				if( distributionParms[ 3 ] > 0.0f )
				{
					...
					if( radiusSqr < distributionParms[ 3 ] * distributionParms[ 3 ] )
					{
						...
						float f = idMath::Sqrt( radiusSqr ) / distributionParms[ 3 ];
						float invf = 1.0f / f;
	...
	else
	{
		...
		switch( customPathType )
		{
			...
			case PPATH_FLIES:  		// ( radialSpeed axialSpeed size )
			{
				...
				angle1 = g->random.RandomFloat() * idMath::PI * 2.0f + customPathParms[ 0 ] * speed1 * g->age;
				angle2 = g->random.RandomFloat() * idMath::PI * 2.0f + customPathParms[ 1 ] * speed1 * g->age;
...
int	idParticleStage::ParticleVerts( particleGen_t* g, idVec3 origin, idDrawVert* verts ) const
{
	...
	// constant rotation
	//
	float angle = ( initialAngle ) ? initialAngle : 360.0f * g->random.RandomFloat();
