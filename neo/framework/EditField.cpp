...
{
	if( autoComplete.length > 0 && autoComplete.length <= idStr::Length( buffer ) )
	{
...
void idEditField::AutoComplete()
{
	...
	if( !autoComplete.valid )
	{
		...
		if( idStr::Length( autoComplete.completionString ) == 0 )
		{
			return;
		}
	...
	else if( autoComplete.matchCount != 1 )
	{
		...
		int buffer_length = idStr::Length( buffer );
		if( autoComplete.length > buffer_length )
		{
			autoComplete.length = buffer_length;
		}
		SetCursor( autoComplete.length );
	}
}
...
void idEditField::Draw( int x, int y, int width, bool showCursor )
{
	...
	int drawLen = widthInChars;
	int len = idStr::Length( buffer ) + 1;
