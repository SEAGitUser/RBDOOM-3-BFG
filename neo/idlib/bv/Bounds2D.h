...
{
	return idBounds2D( idVec2( bounds[ 0 ][ 0 ] * s[ 0 ], bounds[ 0 ][ 1 ] * s[ 1 ] ),
					   idVec2( bounds[ 1 ][ 0 ] * s[ 0 ], bounds[ 1 ][ 1 ] * s[ 1 ] ) );
}
...
ID_INLINE void idBounds2D::MakeValid()
{
	if( bounds[ 0 ].x > bounds[ 1 ].x ) {
		SwapValues( bounds[ 0 ].x, bounds[ 1 ].x );
	}
	if( bounds[ 0 ].y > bounds[ 1 ].y ) {
		SwapValues( bounds[ 0 ].y, bounds[ 1 ].y );
	}
}
