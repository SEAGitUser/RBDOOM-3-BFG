...
class idListNode {
public:
				idListNode() { next = prev = NULL; }

	type *		GetPrev() const { return prev; }
...
template< typename type, idListNode<type> type::*nodePtr >
idLinkedList<type, nodePtr>::idLinkedList()
{
	first = last = NULL;
}
...
template< typename type, idListNode<type> type::*nodePtr >
type *idLinkedList<type, nodePtr>::RemoveFirst()
{
	if( first == NULL )
	{
		return NULL;
...
template< typename type, idListNode<type> type::*nodePtr >
void idLinkedList<type, nodePtr>::Test()
{
	class idMyType {
	public:
