...
void idSurface_Polytope::FromPlanes( const idPlane* planes, const int numPlanes )
{
	...
	windingVerts = ( int* ) _alloca( idFixedWinding::MAX_POINTS * sizeof( int ) );
...
int idSurface_Polytope::SplitPolytope( const idPlane& plane, const float epsilon, idSurface_Polytope** front, idSurface_Polytope** back ) const
{
	...
	*back = polytopeSurfaces[ 1 ];
	...
	for( s = 0; s < 2; s++ )
	{
		...
		v0 = surf->edges[ idMath::Abs( edgeNum ) ].verts[ INT32_SIGNBITSET( edgeNum ) ];
		v1 = surf->edges[ idMath::Abs( edgeNum ) ].verts[ INT32_SIGNBITNOTSET( edgeNum ) ];
		...
		for( i = 1; onPlaneEdges[ s ][ i ] >= 0; i++ )
		{
			for( j = i + 1; onPlaneEdges[ s ][ j ] >= 0; j++ )
			{
				...
				if( v1 == surf->edges[ idMath::Abs( edgeNum ) ].verts[ INT32_SIGNBITSET( edgeNum ) ] )
				{
					v1 = surf->edges[ idMath::Abs( edgeNum ) ].verts[ INT32_SIGNBITNOTSET( edgeNum ) ];
		...
		for( i = 2; onPlaneEdges[ s ][ i ] >= 0; i++ )
		{
			...
			v1 = surf->edges[ idMath::Abs( edgeNum ) ].verts[ INT32_SIGNBITNOTSET( edgeNum ) ];
			v2 = surf->edges[ idMath::Abs( edgeNum ) ].verts[ INT32_SIGNBITSET( edgeNum ) ];
