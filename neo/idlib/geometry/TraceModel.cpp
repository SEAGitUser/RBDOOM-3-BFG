...
void idTraceModel::SetupFrustum( const idBounds &boxBounds, float topOffset )
{
	...
	for( int i = 0; i < numPolys; i++ )
	{
		...
		e2 = polys[ i ].edges[ 2 ];
		e3 = polys[ i ].edges[ 3 ];
		v0 = edges[ idMath::Abs( e0 ) ].v[ INT32_SIGNBITSET( e0 ) ];
		v1 = edges[ idMath::Abs( e0 ) ].v[ INT32_SIGNBITNOTSET( e0 ) ];
		v2 = edges[ idMath::Abs( e1 ) ].v[ INT32_SIGNBITNOTSET( e1 ) ];
		v3 = edges[ idMath::Abs( e2 ) ].v[ INT32_SIGNBITNOTSET( e2 ) ];
		// polygon plane
...
void idTraceModel::SetupDodecahedron( const idBounds& dodBounds )
{
	...
	for( i = 0; i < numPolys; i++ )
	{
		...
		v2 = edges[ idMath::Abs( e1 ) ].v[ INT32_SIGNBITNOTSET( e1 ) ];
		v3 = edges[ idMath::Abs( e2 ) ].v[ INT32_SIGNBITNOTSET( e2 ) ];
		v4 = edges[ idMath::Abs( e3 ) ].v[ INT32_SIGNBITNOTSET( e3 ) ];
		// polygon plane
		polys[ i ].normal = ( verts[ v1 ] - verts[ v0 ] ).Cross( verts[ v2 ] - verts[ v0 ] );
		polys[ i ].normal.Normalize();
		polys[ i ].dist = polys[ i ].normal * verts[ v0 ];
...
void idTraceModel::SetupCylinder( const idBounds &cylBounds, const int numSides, float angleOffset, int axis )
{
	...
	switch( axis ) {
		default:
		case 0: a1 = 0; a2 = 1; a3 = 2; break;
		case 1: a1 = 2; a2 = 0; a3 = 1; break;
		case 2: a1 = 1; a2 = 2; a3 = 0; break;
	}
	...
	for( i = 0; i < n; i++ )
	{
		// verts
		float s, c;
		idMath::SinCos( ( idMath::TWO_PI * i / n ) + DEG2RAD( angleOffset ), s, c );
		verts[ i ][ a1 ] = c * halfSize[ a1 ] + offset[ a1 ];
		verts[ i ][ a2 ] = s * halfSize[ a2 ] + offset[ a2 ];
...
void idTraceModel::SetupCone( const idBounds& coneBounds, const int numSides )
{
	...
	for( i = 0; i < n; i++ )
	{
		...
		verts[ i ].x = c * halfSize.x + offset.x;
		verts[ i ].y = s * halfSize.y + offset.y;
...
void idTraceModel::SetupBone( const float length, const float width )
{
	...
	for( i = 0; i < 6; i++ )
	{
		polys[ i ].dist = polys[ i ].normal * verts[ edges[ idMath::Abs( polys[ i ].edges[ 0 ] ) ].v[ 0 ] ];
		polys[ i ].bounds.Clear();
		for( j = 0; j < 3; j++ )
		{
			edgeNum = polys[ i ].edges[ j ];
			polys[ i ].bounds.AddPoint( verts[ edges[ idMath::Abs( edgeNum ) ].v[ edgeNum < 0 ] ] );
		}
	}
...
{
	idVec3* verts = ( idVec3* )_alloca16( w.GetNumPoints() * sizeof( idVec3 ) );
...
void idTraceModel::Rotate( const idMat3& rotation )
{
	...
	for( int i = 0; i < numPolys; i++ )
	{
		...
		{
			edgeNum = polys[ i ].edges[ j ];
			polys[ i ].bounds.AddPoint( verts[ edges[ idMath::Abs( edgeNum ) ].v[ INT32_SIGNBITSET( edgeNum ) ] ] );
		}
		polys[ i ].dist = polys[ i ].normal * verts[ edges[ idMath::Abs( edgeNum ) ].v[ INT32_SIGNBITSET( edgeNum ) ] ];
...
void idTraceModel::Shrink( const float m )
{
	if( type == TRM_POLYGON )
	{
		for( int i = 0; i < numEdges; i++ )
		{
			int edgeNum = polys[ 0 ].edges[ i ];
			const auto & edge = edges[ idMath::Abs( edgeNum ) ];
			idVec3 dir = verts[ edge.v[ INT32_SIGNBITSET( edgeNum ) ] ] - verts[ edge.v[ INT32_SIGNBITNOTSET( edgeNum ) ] ];
	...
	for( int i = 0; i < numPolys; ++i )
	{
		...
		for( int j = 0; j < polys[ i ].numEdges; ++j )
		{
			int edgeNum = polys[ i ].edges[ j ];
			auto edge = &edges[ idMath::Abs( edgeNum ) ];
...
{
	for( int i = numVerts; i < MAX_TRACEMODEL_VERTS; i++ )
	{
		verts[ i ].Zero();
	}
	memset( &edges[ 0 ], 0, sizeof( edges[ 0 ] ) );
	for( int i = numEdges + 1; i < MAX_TRACEMODEL_EDGES + 1; i++ )
	{
		memset( &edges[ i ], 0, sizeof( edges[ i ] ) );
	}
	for( int i = 0; i < numPolys; i++ )
	{
		for( int j = polys[ i ].numEdges; j < MAX_TRACEMODEL_POLYEDGES; j++ )
		{
			polys[ i ].edges[ j ] = 0;
		}
	}
	for( int i = numPolys; i < MAX_TRACEMODEL_POLYS; i++ )
	{
...
bool idTraceModel::Verify()
{
	...
	for( int i = 0; i < numPolys; ++i )
	{
		...
		for( int j = 0; j < polys[ i ].numEdges; ++j )
		{
			edgeNum = poly->edges[ j ];
			edge = &edges[ idMath::Abs( edgeNum ) ];
			vertexNum = edge->v[ INT32_SIGNBITSET( edgeNum ) ];
			float d = poly->normal * verts[ vertexNum ] - poly->dist;
			if( idMath::Fabs( d ) > 1e-4f )
			{
				return false;
			}
		}
...
bool idTraceModel::IsClosedSurface() const
{
	...
	for( int i = 0; i < numPolys; ++i )
	{
		for( int j = 0; j < polys[ i ].numEdges; ++j )
		{
			numEdgeUsers[ idMath::Abs( polys[ i ].edges[ j ] ) ]++;
		}
	}
...
float idTraceModel::GetPolygonArea( int polyNum ) const
{
	...
	poly = &polys[ polyNum ];
	total = 0.0f;
	base = verts[ edges[ idMath::Abs( poly->edges[ 0 ] ) ].v[ INT32_SIGNBITSET( poly->edges[ 0 ] ) ] ];
	for( int i = 0; i < poly->numEdges; ++i )
	{
		v1 = verts[ edges[ idMath::Abs( poly->edges[ i ] ) ].v[ INT32_SIGNBITSET( poly->edges[ i ] ) ] ] - base;
		v2 = verts[ edges[ idMath::Abs( poly->edges[ i ] ) ].v[ INT32_SIGNBITNOTSET( poly->edges[ i ] ) ] ] - base;
		cross = v1.Cross( v2 );
		total += cross.Length();
	}
	return total * 0.5f;
...
int idTraceModel::GetProjectionSilhouetteEdges( const idVec3& projectionOrigin, int silEdges[ MAX_TRACEMODEL_EDGES ] ) const
{
	...
	for( i = 0; i < numPolys; i++ )
	{
		...
		dir = verts[ edges[ idMath::Abs( edgeNum ) ].v[ INT32_SIGNBITSET( edgeNum ) ] ] - projectionOrigin;
		if( dir * poly->normal < 0.0f )
		{
			for( j = 0; j < poly->numEdges; j++ )
			{
				edgeNum = poly->edges[ j ];
				edgeIsSilEdge[ idMath::Abs( edgeNum ) ] ^= 1;
			}
		}
	}
...
============
*/
void idTraceModel::ProjectionIntegrals( int polyNum, int a, int b, struct projectionIntegrals_t& integrals ) const
{
	const traceModelPoly_t* poly;
	int i, edgeNum;
	idVec3 v1, v2;
	...
	for( i = 0; i < poly->numEdges; i++ )
	{
		...
		v1 = verts[ edges[ idMath::Abs( edgeNum ) ].v[ edgeNum < 0 ] ];
		v2 = verts[ edges[ idMath::Abs( edgeNum ) ].v[ edgeNum > 0 ] ];
		a0 = v1[ a ];
		b0 = v1[ b ];
		a1 = v2[ a ];
		b1 = v2[ b ];
