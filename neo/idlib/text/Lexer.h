...
public:
	...
	bool					Parse1DMatrix( int x, float *m, bool expectCommas = false );
	// RB begin
	bool					Parse1DMatrixJSON( int x, float* m );
	// RB end
	...
	ID_INLINE bool			HadError() const { return hadError; }
	// returns true if any warnings were printed
	ID_INLINE bool			HadWarning() const { return hadWarning; }
...
ID_INLINE const char *idLexer::GetFileName() const
{
	return this->filename;
}

ID_INLINE int idLexer::GetFileOffset() const
{
	return this->script_p - this->buffer;
}

ID_INLINE int idLexer::GetLastFileOffset() const
{
	return this->lastScript_p - this->buffer;
}

ID_INLINE int idLexer::GetFileSize() const
{
...
{
	release_assert( !loaded ); // all flags must be set before loading the file
	this->flags = flags;
}
