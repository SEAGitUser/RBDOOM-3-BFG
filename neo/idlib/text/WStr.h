...
// character methods
	...
	static void			ShowMemoryUsage_f( const class idCmdArgs &args );

	static void			SetStringAllocator( wideStringDataAllocator_t* allocator );
	static wideStringDataAllocator_t* GetStringAllocator();
...
ID_INLINE size_t idWStr::Size() const
{
	return sizeof( *this ) + Allocated() * sizeof( wchar_t );
}

ID_INLINE const wchar_t *idWStr::c_str() const
{
	return data;
}
...
ID_INLINE int idWStr::Length() const
{
	return len;
}

ID_INLINE int idWStr::Allocated() const
{
	if( data != baseBuffer )
	{
		return alloced;
...
ID_INLINE bool idWStr::IsEmpty() const
{
	return ( Cmp( data, L"" ) == 0 );
}

ID_INLINE void idWStr::Clear()
{
	FreeData();
	Init();
}
...
ID_INLINE bool idWStr::IsColor() const
{
	return IsColor( data );
}

ID_INLINE idWStr &idWStr::RemoveColors()
{
	RemoveColors( data );
	len = Length( data );
	return *this;
...
ID_INLINE int idWStr::LengthWithoutColors() const
{
