...
class idWToken : public idWStr//, public idPoolAllocator< idWToken, idPoolAllocator_DefaultIdentifier, 128 > 
{
...
ID_INLINE double idWToken::GetDoubleValue()
{
	if( type != TT_NUMBER )
	{
		return 0.0;
...
ID_INLINE float idWToken::GetFloatValue()
{
	return ( float )GetDoubleValue();
}

ID_INLINE unsigned long	idWToken::GetUnsignedLongValue()
{
	if( type != TT_NUMBER )
	{
		return 0;
...
ID_INLINE int idWToken::GetIntValue()
{
	return ( int )GetUnsignedLongValue();
}

ID_INLINE int idWToken::WhiteSpaceBeforeToken() const
{
...
ID_INLINE void idWToken::SetIntValue( unsigned int intvalue )
{
