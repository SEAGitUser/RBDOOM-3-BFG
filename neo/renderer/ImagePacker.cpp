...
public:
	...
	idImagePackerNode( const idImagePackerNode &other )
	{
		...
		if( other.children[ 0 ] )
		{
			children[ 0 ] = new( TAG_IMAGE ) idImagePackerNode( *other.children[ 0 ] );
		}
		else
		{
			children[ 0 ] = NULL;
		...
		if( other.children[ 1 ] )
		{
			children[ 1 ] = new( TAG_IMAGE ) idImagePackerNode( *other.children[ 1 ] );
		}
		else
		{
			children[ 1 ] = NULL;
...
idImagePackerNode* idImagePackerNode::Insert( const idSubImage &image )
{
	...
	else
	{
		...
		// Split
		children[ 0 ] = new( TAG_IMAGE ) idImagePackerNode;
		children[ 1 ] = new( TAG_IMAGE ) idImagePackerNode;
...
idImagePacker::idImagePacker( int width, int height )
{
	...
	root = new( TAG_IMAGE ) idImagePackerNode;
	root->rect.x = 0;
...
{
	root = new( TAG_IMAGE ) idImagePackerNode( *other.root );
...
idSubImage idImagePacker::PackImage( int width, int height, bool expandIfFull )
{
	...
	if( !resultNode )
	{
		...
		idImagePackerNode *newRoot = new( TAG_IMAGE ) idImagePackerNode;
		idImagePackerNode *newChild1 = new( TAG_IMAGE ) idImagePackerNode;
