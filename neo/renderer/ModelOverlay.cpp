...
void idRenderModelOverlay::CreateOverlay( const idRenderModel* model, const idPlane localTextureAxis[2], const idMaterial* material )
{
	...
	for( int surfNum = 0; surfNum < model->NumBaseSurfaces(); surfNum++ )
	{
		...
		if( !surf->GetTriangles() || !surf->GetMaterial() )
		{
			continue;
		}

		// some surfaces can explicitly disallow overlays
		if( !surf->GetMaterial()->AllowOverlays() )
		{
			continue;
		}

		const auto * const tri = surf->GetTriangles();
		...
		auto & overlay = overlays[ nextOverlay++ & ( MAX_OVERLAYS - 1 ) ];
		FreeOverlay( overlay );
...
void idRenderModelOverlay::CreateDeferredOverlays( const idRenderModel* model )
{
	for( unsigned int i = firstDeferredOverlay; i < nextDeferredOverlay; i++ )
	{
		...
		if( parms.startTime > tr.viewDef->GetGameTimeMS() -  DEFFERED_OVERLAY_TIMEOUT )
		{
...
unsigned int idRenderModelOverlay::GetNumOverlayDrawSurfs()
{
	...
	{
		const auto & overlay = overlays[i & ( MAX_OVERLAYS - 1 )];
...
drawSurf_t* idRenderModelOverlay::CreateOverlayDrawSurf( const viewModel_t* space, const idRenderModel* baseModel, unsigned int index )
{
	...
	auto staticModel = static_cast< const idRenderModelStatic* >( baseModel );
	auto material = overlayMaterials[index];
	...
	auto mappedVerts = ( idDrawVert* )vertexCache.MappedVertexBuffer( newTri->vertexCache );
	auto mappedIndexes = ( triIndex_t* )vertexCache.MappedIndexBuffer( newTri->indexCache );
	...
	for( unsigned int i = firstOverlay; i < nextOverlay; ++i )
	{
		auto & overlay = overlays[i & ( MAX_OVERLAYS - 1 )];
		...
		const auto * baseSurf = ( overlay.surfaceNum < staticModel->NumSurfaces() )? staticModel->Surface( overlay.surfaceNum ) : nullptr;
		...
		const auto * const baseTri = baseSurf->GetTriangles();
