...
// the renderEntityParms_t::joints array needs to point at enough memory to store the number of joints rounded up to two for SIMD
...
struct renderEntityParms_t
{
	...
	// texturing
	const idMaterial * 		customMaterial;			// if non-0, all surfaces will use this
	const idMaterial * 		referenceMaterial;		// used so flares can reference the proper light shader
	...
	ID_INLINE void			SetColorParm( float r, float g, float b, float a )
	{
		shaderParms[ SHADERPARM_RED ]   = r;
		shaderParms[ SHADERPARM_GREEN ] = g;
...
public:
	...
	virtual	qhandle_t		AddEntityDef( const renderEntityParms_t* ) = 0;
	virtual	void			UpdateEntityDef( qhandle_t entityHandle, const renderEntityParms_t* ) = 0;
	virtual	void			FreeEntityDef( qhandle_t entityHandle ) = 0;
	virtual const renderEntityParms_t * GetRenderEntityParms( qhandle_t entityHandle ) const = 0;

	virtual	qhandle_t		AddLightDef( const renderLightParms_t* ) = 0;
	virtual	void			UpdateLightDef( qhandle_t lightHandle, const renderLightParms_t* ) = 0;
	virtual	void			FreeLightDef( qhandle_t lightHandle ) = 0;
	virtual const renderLightParms_t * GetRenderLightParms( qhandle_t lightHandle ) const = 0;
	...
	virtual void			ProjectDecalOntoWorld( const idFixedWinding&, const idVec3& projectionOrigin, const bool parallel, const float fadeDepth, const idMaterial*, const int startTime ) = 0;

	// Creates decals on static models.
	virtual void			ProjectDecal( qhandle_t entityHandle, const idFixedWinding&, const idVec3& projectionOrigin, const bool parallel, const float fadeDepth, const idMaterial*, const int startTime ) = 0;

	// Creates overlays on dynamic models.
	virtual void			ProjectOverlay( qhandle_t entityHandle, const idPlane localTextureAxis[2], const idMaterial*, const int startTime ) = 0;
	...
	virtual void			SetRenderView( const renderViewParms_t* ) = 0;
	...
	virtual void			RenderScene( const renderViewParms_t* ) = 0;
	...
	virtual	qhandle_t		FindPortal( const idBounds& ) const = 0;
	...
	virtual	bool			AreasAreConnected( int areaNum1, int areaNum2, portalConnection_t ) const = 0;
	...
	virtual bool			ModelTrace( modelTrace_t&, qhandle_t entityHandle, const idVec3& start, const idVec3& end, const float radius ) const = 0;

	// Traces vs the whole rendered world. FIXME: we need some kind of material flags.
	virtual bool			Trace( modelTrace_t&, const idVec3& start, const idVec3& end, const float radius, bool skipDynamic = true, bool skipPlayer = false ) const = 0;

	// Traces vs the world model bsp tree.
	virtual bool			FastWorldTrace( modelTrace_t&, const idVec3& start, const idVec3& end ) const = 0;

	//-------------- Demo Control  -----------------

	// Writes a loadmap command to the demo, and clears archive counters.
	virtual void			StartWritingDemo( idDemoFile* ) = 0;
	...
	virtual bool			ProcessDemoCommand( idDemoFile*, renderViewParms_t*, int* demoTimeOffset ) = 0;
