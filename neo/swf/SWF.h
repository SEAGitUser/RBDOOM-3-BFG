...
private:
	...
	void			RenderMask( const swfDisplayEntry_t* mask, const swfRenderState_t& renderState, const int stencilMode );
	...
	void			DrawEditCursor( float x, float y, float w, float h, const swfMatrix_t& matrix );
	void			DrawLine( const idVec2& p1, const idVec2& p2, float width, const swfMatrix_t& matrix );
	...
	void			DrawRect( const swfRect_t& rect, const idVec4& color );
	int				DrawText( float x, float y, float scale, idVec4 color, const char* text, float adjust, int limit, int style );
	int				DrawText( const char* text, float textScale, int textAlign, idVec4 color, const swfRect_t& rectDraw, bool wrap, int cursor = -1, bool calcOnly = false, idList<int>* breaks = NULL, int limit = 0 );
	// RB end
