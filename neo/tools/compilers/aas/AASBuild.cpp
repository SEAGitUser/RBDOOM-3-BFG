...
void idAASBuild::ParseProcNodes( idLexer & src )
{
	...
	{
		auto node = &( idAASBuild::procNodes[ i ] );
...
============
*/
static bool MergeAllowed( idBrush* b1, idBrush* b2 )
{
...
============
*/
static bool ExpandedChopAllowed( idBrush* b1, idBrush* b2 )
{
...
============
*/
static bool ExpandedMergeAllowed( idBrush* b1, idBrush* b2 )
{
	return( b1->GetContents() == b2->GetContents() );
}
...
void RunAASDir_f( const idCmdArgs& args )
{
	...
	// scan for .map files
	auto mapFiles = fileSystem->ListFiles( idStr( "maps/" ) + args.Argv( 1 ), ".map" );

	// create AAS files for all the .map files
	for( int i = 0; i < mapFiles->GetNumFiles(); i++ )
	{
		if( i ) {
			common->Printf( "=======================================================\n" );
		}
