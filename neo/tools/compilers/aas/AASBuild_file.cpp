...
{
	int x = ( ( ( int )( vec[0] - aas_vertexBounds[0].x + 0.5 ) ) + 2 ) >> 2;
	int y = ( ( ( int )( vec[1] - aas_vertexBounds[0].y + 0.5 ) ) + 2 ) >> 2;
...
bool idAASBuild::GetVertex( const idVec3& v, int* vertexNum )
{
	...
	int hashKey = idAASBuild::HashVec( vert );
...
void idAASBuild::GetSizeEstimate_r( idBrushBSPNode* parent, idBrushBSPNode* node, sizeEstimate_t & size )
{
	idBrushBSPPortal* p;
	int s;
