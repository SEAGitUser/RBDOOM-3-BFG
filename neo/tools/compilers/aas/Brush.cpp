...
{
	auto side = new idBrushSide( plane, planeNum );
	side->flags = flags;
	if( winding )
	{
...
bool idBrush::Subtract( const idBrush* b, idBrushList& list ) const
{
	...
	list.Clear();
	const idBrush* in = this;
	for( int i = 0; i < b->sides.Num() && in; i++ )
	{
...
{
	auto map = new idBrushMap( fileName, ext );
...
void idBrushMap::WriteBrushList( const idBrushList& brushList )
{
	...
	for( auto b = brushList.Head(); b; b = b->Next() )
	{
		WriteBrush( b );
	}
}
