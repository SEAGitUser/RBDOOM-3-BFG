...
bool idBrushBSPNode::Split( const idPlane& splitPlane, int splitPlaneNum )
{
	...
	auto mid = new idWinding( splitPlane.Normal(), splitPlane.Dist() );
...
int idBrushBSP::BrushSplitterStats( const idBrush* brush, int planeNum, const idPlaneSet& planeList, bool* testedPlanes, splitterStats_t & stats )
{
...
============
*/
int idBrushBSP::FindSplitter( idBrushBSPNode* node, const idPlaneSet& planeList, bool* testedPlanes, splitterStats_t & bestStats )
{
...
{
	for( auto brush = node->brushList.Head(); brush; brush = brush->Next() )
	{
		if( !( brush->GetSavedPlaneSide() & BRUSH_PLANESIDE_FACING ) )
		{
			continue;
		}
		int numValidBrushSplitters = 0;
...
idBrushBSPNode* idBrushBSP::BuildBrushBSP_r( idBrushBSPNode* node, const idPlaneSet& planeList, bool* testedPlanes, int skipContents )
{
	...
	int planeNum = FindSplitter( node, planeList, testedPlanes, bestStats );
...
{
	auto w = new idWinding( node->plane.Normal(), node->plane.Dist() );

	// clip by all the parents
	for( auto n = node->parent; n && w; n = n->parent )
	{
