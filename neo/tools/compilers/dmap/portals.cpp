...
uPortal_t*	AllocPortal()
{
	...
	auto p = ( uPortal_t* )Mem_Alloc( sizeof( uPortal_t ), TAG_TOOLS );
...
{
	auto w = new idWinding( dmapGlobals.mapPlanes[node->planenum] );

	// clip by all the parents
	for( auto n = node->parent ; n && w ; )
	{
...
static void SplitNodePortals( node_t* node )
{
	...
	auto plane = &dmapGlobals.mapPlanes[node->planenum];
	f = node->children[0];
	b = node->children[1];
...
bool FloodEntities( tree_t* tree )
{
	...
	{
		auto mapEnt = dmapGlobals.uEntities[ i ].mapEntity;
