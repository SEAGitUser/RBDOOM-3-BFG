...
void	FixGlobalTjunctions( uEntity_t* e )
{
	...
	if( dmapGlobals.entityNum == 0 )
	{
		for( int eNum = 1; eNum < dmapGlobals.num_entities; ++eNum )
		{
			...
			auto model = renderModelManager->FindModel( modelName );
			...
			{
				const auto surface = model->Surface( i );
				const auto tri = surface->GetTriangles();

				mapTri_t mapTri;
				memset( &mapTri, 0, sizeof( mapTri ) );
				mapTri.material = surface->GetMaterial();
