...
void idDeviceContext::DrawWinding( idWinding& w, const idMaterial* mat )
{
	...
	auto verts = renderSystem->AllocTris( w.GetNumPoints(), tempIndexes, numIndexes, mat );
	if( verts == NULL )
		return;
...
void idDeviceContext::DrawMaterial( float x, float y, float w, float h, const idMaterial* mat, const idColor & color, float scalex, float scaley )
{
	renderSystem->SetColor( color.ToVec4() );
...
void idDeviceContext::DrawMaterialRotated( float x, float y, float w, float h, const idMaterial* mat, const idColor & color, float scalex, float scaley, float angle )
{
	renderSystem->SetColor( color.ToVec4() );
...
void idDeviceContext::DrawFilledRect( float x, float y, float w, float h, const idColor & color )
{
	if( color.a == 0.0f )
		return;
...
void idDeviceContext::DrawRect( float x, float y, float w, float h, float size, const idColor & color )
{
	if( color.a == 0.0f )
		return;
...
void idDeviceContext::DrawMaterialRect( float x, float y, float w, float h, float size, const idMaterial* mat, const idColor & color )
{
	if( color.a == 0.0f )
		return;
...
void idDeviceContext::SetCursor( int n )
{
	...
	{
		auto binds = idKeyInput::KeyBindingsFromBinding( "_use", true );
...
void idDeviceContext::DrawCursor( float* x, float* y, float size )
{
	...
	renderSystem->SetColor( idColor::white.ToVec4() );
...
void idDeviceContext::PaintChar( float x, float y, const scaledGlyphInfo_t& glyphInfo )
{
	...
	auto material = glyphInfo.material;
	...
	DrawStretchPic( x, y, w, h, s, t, s2, t2, material );
}

int idDeviceContext::DrawText( float x, float y, float scale, const idColor & color, const char* text, float adjust, int limit, int style, int cursor )
{
	idStr drawText = text;
...
int idDeviceContextOptimized::DrawText( float x, float y, float scale, const idColor & color, const char* text, float adjust, int limit, int style, int cursor )
{
	...
	auto currentColorNativeByteOrder = idColor::PackColor( color.ToVec4() );
	...
	while( charIndex < drawText.Length() )
	{
		...
		if( textChar == C_COLOR_ESCAPE )
		{
			...
			currentColorNativeByteOrder = idColor::PackColor( newColor );
			continue;
		}
		...
		if( !ClippedCoords( &drawX, &drawY, &w, &h, &s, &t, &s2, &t2 ) )
		{
			...
			auto verts = tr_guiModel->AllocTris( 4, idTriangles::quadIndexes, 6, glyphInfo.material, 0, STEREO_DEPTH_TYPE_NONE );
			///auto verts = renderSystem->AllocTris( 4, quadPicIndexes, 6, glyphInfo.material, STEREO_DEPTH_TYPE_NONE );
			if( verts != NULL )
			{
			#if 0
				...
				verts[ 3 ].ClearColor2();
			#else
				localVerts[ 0 ].SetPosition( x1, y1, 0.0f );
				localVerts[ 0 ].SetTexCoord( s, t );
				localVerts[ 0 ].SetNativeOrderColor( currentColorNativeByteOrder );
				localVerts[ 0 ].ClearColor2();

				localVerts[ 1 ].SetPosition( x2, y1, 0.0f );
				localVerts[ 1 ].SetTexCoord( s2, t );
				localVerts[ 1 ].SetNativeOrderColor( currentColorNativeByteOrder );
				localVerts[ 1 ].ClearColor2();

				localVerts[ 2 ].SetPosition( x2, y2, 0.0f );
				localVerts[ 2 ].SetTexCoord( s2, t2 );
				localVerts[ 2 ].SetNativeOrderColor( currentColorNativeByteOrder );
				localVerts[ 2 ].ClearColor2();

				localVerts[ 3 ].SetPosition( x1, y2, 0.0f );
				localVerts[ 3 ].SetTexCoord( s, t2 );
				localVerts[ 3 ].SetNativeOrderColor( currentColorNativeByteOrder );
