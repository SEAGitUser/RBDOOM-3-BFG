...
intptr_t idWindow::ParseTerm( idTokenParser* src, idWinVar* var, intptr_t component )
{
	...
	auto table = declManager->FindType( DECL_TABLE, token.c_str(), false )->Cast<idDeclTable>();
	if( table )
	{
		a = table->Index();
	...
	if( var )
	{
		...
		var->Init( token, this );
		b = component;
		if( var->Cast<idWinVec4>() ) // dynamic_cast< idWinVec4* >( var ) )
		{
			if( src->ReadToken( &token ) )
			{
				if( token == "[" )
...
================
*/
bool idWindow::ContainsStateVars() const
{
	if( updateVars.Num() )
	{
		return true;
...
================
*/
bool idWindow::Interactive() const
{
	if( scripts[ ON_ACTION ] )
	{
		return true;
...
================
*/
const idWindow * idWindow::FindChildByPoint( float x, float y, idWindow** below ) const
{
	int c = children.Num();
	...
	for( int i = c - 1; i >= 0; i-- )
	{
		auto found = children[ i ]->FindChildByPoint( x, y, below );
		if( found )
		{
			if( *below )
			{
